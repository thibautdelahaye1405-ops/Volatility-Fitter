...
export const WHATS_NEW: WhatsNewEntry[] = [
  ...
  },
  {
    ...
    items: [
      ...
      "Surface, Term and Lanes stages: one σ(k, τ) sheet per lane with a shared camera or a signed difference against the production lane; the term structure with every lane's ATM and var-swap vols; the Lanes evidence page — a metric across frames, the summary table with the handle-path roughness (what a prior or a filter damps, at the rms cost beside it) and a lane's filter ring with the playhead as cursor.",
      "Series files: **Export** downloads a `.volfit-series.json` (frames, lanes, fits, carried state); **File ▸ Open series…** or a drop on the shell recreates it on any desk. **Adopt as prior** saves the production lane's fits at the playhead's frame as the ticker's live prior (save = activate).",
