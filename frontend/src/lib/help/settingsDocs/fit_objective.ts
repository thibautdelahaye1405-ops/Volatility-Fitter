...
export const FIT_OBJECTIVE_DOCS: SettingDoc[] = [
  ...
  },
  {
    ...
    summary: "How much each quote counts in the objective: one vote per quote, or a TARGET weight distribution over log-strike (uniform, time value, vega or delta) reached through a strike-density correction.",
    details:
      "`equal` (default, the historical scheme) gives every quote's residual the same weight — one vote per quote, so the aggregate weight along the smile follows the exchange's listing grid (dense near the money, sparse in the wings). Every other scheme names a **target** shape and multiplies it by the quote's Voronoi cell width in log-strike (the room that quote alone represents), so a crowded strike region does not outvote the wings just by having more listings: `uniform_density` is the flat target — the bare density correction, so strikes listed every 5 points weigh like 1/K and the summed weight is uniform in log-strike whatever the grid; `tv_density` follows time value (fastest wing decay), `vega_density` follows Black vega (flattest — the natural choice when the target metric is vol error) and `delta_density` follows the OTM |forward delta| (in between). The spacing multiplier is capped at 10× so one isolated far-wing quote cannot dominate.\n\n" +
      "Weights are normalized to mean 1, so switching schemes never changes the balance against `regLambda` or `sigmoidRidge`. It applies in every fit target and to every model, and it is orthogonal to `fitMode`: the mode chooses each quote's target, the scheme chooses how much each quote matters. The Parametric **Weights** layer draws, under the smile and on its x axis, the target shape beside the weight the fit actually sums; hovering a bar reads the multiplier applied.",
    example:
      "Switch `weightScheme` to `vega_density` on a 60-quote SPY slice with dense 5-point strikes near ATM: the ATM cluster's aggregate weight falls, the 10Δ put and call quotes gain relative weight, and the wing RMS in Quality improves by a few bp while the ATM RMS worsens slightly. Switch to `uniform_density` on the same slice: the target strip is flat and the weight strip alone decays across the crowded upper strikes.",
    cacheEffect: "fit-version",
    surfaced: true,
