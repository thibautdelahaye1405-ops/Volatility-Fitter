...
| 14 Graph | `14_graph_messages.tex` | every edge is a contract: standalone doc of the precision-message system (no old-operator-first narrative; smoothing mode = neutral appendix); edges as Gaussian contracts, adjudicated amplitude, the global solve as accountant | `gen_graph_messages.py` |
