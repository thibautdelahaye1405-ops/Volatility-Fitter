...
structurally, SVI by a soft cap, Multi-Core SIV by wing-neutral hats --- and
prove the zero-wing inheritance the SIV design relies on. The finite wing is a
...
The Multi-Core SIV slice (Note~03) is a convex base plus up to two
\emph{zero-wing} hats.
...
quotes. \Cref{tab:betas} collects the analytic slopes.
...
%======================================================================
\section{Policing the finite wing: the SIV put-wing regularizer}
\label{sec:r6}

An admissible slope does not make an admissible wing. The backtest's arb
census located the Multi-Core SIV's failures precisely: $64\%$ of butterfly
...
\begin{example}[title={Case file: the SIV that invented a put wing}]
\textbf{Setup.} The three-regime backtest sweep (F4): Multi-Core SIV on
illiquid single names, where the put wing has a handful of wide quotes.

\textbf{Failure mode.} SIV-2 improved in-sample RMS over SIV-0 --- and
...
positivity confirmed) while SIV-2's survived ($76\%$, genuine). The census
...
RMS improved to $25$ bp --- the SIV had been chasing arbitraged de-Am
...
\emph{Complementary, not redundant}: the input repair makes the output
penalty affordable. On liquid names both are byte-identical no-ops.
\end{example}
...
repair revert (Note~05), and the SIV put-wing penalty above --- resolve into
one rule.
...
SIV Durrleman $g\ge0$ & one & \textbf{extended} $\pm2$ ATM-std past quotes &
\cref{sec:r6}\\
...
\texttt{nCores} & $2$ (max $2$) & SIV core count, schema-clamped
(\cref{sec:r6}).\\
...
endpoint scales; SVI/SIV slopes are closed forms of the fitted parameters;
