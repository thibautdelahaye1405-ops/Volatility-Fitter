...
admissible slope (a butterfly arbitrage in the extrapolated region). This
note owns the strike axis \emph{beyond} the quotes; its complement on the
traded range is Note~02's belly butterfly certificate, which hard-gates
publish. Together the coverage is complete: certificate on the traded range
(with teeth), advisory measurement and optional tapered enforcement beyond
it, and a wing-only projection at publish --- no part of the displayed
strike axis is uninspected.
...
min-variance penalty. The cap $\beta_{\max}$ is deliberately \emph{not} the
bound itself: \texttt{leeSlopeMax} defaults to $1.95$, strictly buffered,
because the boundary case is broken --- a slice sitting exactly \emph{at}
$\beta=2$ passes the hinge with zero penalty yet already carries negative
tail density (the committee counterexample reads $g(10)=-0.0485$, and the
trap was live on a production SPY wing; certification case
\texttt{svi\_lee\_boundary}, Note~02). \Cref{lem:psi}'s ceiling is a
necessary condition, and a fence must not end on its broken boundary. On a
strictly admissible smile both penalties are exactly zero, and under the
default structural chart (Note~02) both wings are strictly below the cap at
every iterate --- the fence is inert by construction.
...
cannot change the wings --- the entire point of the zero-wing design. Two
caveats bound what the proposition buys. First, it pins only the asymptotic
\emph{slope}: a hat can still dent the Durrleman $g$ below zero at
\emph{finite} $|z|$, out in the sparsely quoted put wing --- a residual
failure mode that is real, measured, and policed by the put-wing regularizer
of \cref{sec:r6}. Second, wing-\emph{neutral} is not wing-\emph{admissible}:
the inheritance theorem preserves whatever slopes the base fitted, and
nothing in the MCS calibration bounds forces those base slopes to satisfy
Lee's sign and cap conditions, $0\le\beta_{L,R}\le2$. Unlike LQD (structural)
and SVI (soft cap), MCS admissibility of the wings is \emph{diagnosed}, not
enforced.
...
promise is that each contract's Lee status is \emph{stated and checked},
with per-model teeth: structural for LQD, a soft cap for SVI, bounded local
variance for LV, and \emph{diagnosis only} for the MCS base slopes.
...
construction; see the matching open problem there. \emph{Phase 1 (2026-07-09):
the Quality report now measures this region --- worst Durrleman $g$, calendar
crossings and wing-slope order over the time-value envelope, advisory only
(never gating readiness). Phase 2 (same day): opt-in tapered enforcement
(\texttt{extrapEnforce}, OFF by default) adds exactly these three hinges to
the SVI/MCS overlay fits, budgeted in vol units at a fraction of the data
weight so it leans without outvoting quotes; the far-field calendar order is
enforced through the SCALAR wing-slope condition, never pointwise --- and
under the symmetric surface solver the same toggle arms the LQD \emph{tail
contract}: per-interface seam price ordering plus linear wing-slope
(log-endpoint-scale) ordering rows in the joint repair (Note~10). Note~10's
matching remark carries the mechanism and the measured lean/cost trade.
Phase 3 (2026-07-10): the publish-time wing-only projection --- exported
curve samples lifted onto the discrete arbitrage-free cone in OTM-price
space, traded core pinned byte-identically, fits and in-app views untouched
(\nolinkurl{volfit/models/projection.py}); the projection is a repair, so
its \emph{authority} is confined to the wings per \cref{sec:principle} even
though the constraints it restores extend there, and it is audited to
introduce no calendar crossings (the manifest's
\texttt{projectionCalendarWorstBp}). The three phases are certification case
\texttt{extrap\_wing\_contracts}.}
\end{remark}
...
\addlinespace
Lee cap strictly buffered ($\beta=2$ boundary broken; cert.\
\texttt{svi\_lee\_boundary}) &
\cref{sec:models} &
\nolinkurl{models/svi_jw/calibrate.py} &
\nolinkurl{test_svi_lee_boundary.py}\\
Extrapolated-region phases 1--3 (cert.\ \texttt{extrap\_wing\_contracts}) &
\cref{rem:softwings} &
\nolinkurl{models/projection.py}, \nolinkurl{calib/extrap.py} &
\nolinkurl{test_wing_projection.py}, \nolinkurl{test_extrap_enforce.py},
\nolinkurl{test_diagnostics.py}\\
\bottomrule
\end{longtable}
\endgroup
...
(\cref{sec:r6}).\\
\texttt{leeSlopeMax} & $1.95$ & SVI Lee cap $b(1+|\rho|)\le\beta_{\max}$,
strictly buffered under Lee's bound ($\beta=2$ is the broken boundary;
\cref{sec:models}).\\
