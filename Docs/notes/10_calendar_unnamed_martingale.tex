...
\input{figures/cal_martingale_tables.tex}
...
not $k$).  As a \emph{fit-time} constraint the same face \emph{used to}
enter as a soft squared hinge on a strided subgrid ($\sim$320 of the
$8001$ nodes),
\begin{equation}
\sqrt{W_{\mathrm{cal}}}\,
...
Hold \eqref{eq:slack} in mind as the \emph{historical} enforcement form
--- it is the one \cref{sec:where} convicts.  Production has abandoned
this face for enforcement entirely and keeps it as the free, wing-dense
\emph{diagnostic} it is perfectly suited to be; the quantile form
survives only as a legacy threading option.
...
commit path, the full-surface loop and the coupled workflow (this is the
sequential solver's route; under the shipped symmetric default the
surface's interfaces carry the confined price rows of
\cref{sec:coordinate}).  Wherever
...
data and inconsistent only in extrapolation, and registered in the named
certification pack as \texttt{calendar\_phantom} --- the founding
incident keeps its own lock.
\end{example}
...
witnesses it.  What that region deserves remains an explicitly flagged
open problem (2026-07) on \emph{doctrine}; the \emph{machinery} has
shipped in three graduated phases, certification-locked together as case
\texttt{extrap\_wing\_contracts}.  \emph{Measure} (always on): the quality report
...
rather than silently repaired.  The export manifest audits the repair
via \texttt{projectionCalendarWorstBp} --- the field that licenses a
price-moving repair at the boundary at all.  A repair moves prices, so
its \emph{authority} is confined to the wings even though the constraints
...
scheme.  And \emph{the extrapolated region remains an open doctrine
question}
(\cref{sec:extrap}): measured always, leaned on only opt-in, repaired
only at the published boundary --- the machinery itself is shipped and
...
certification case \texttt{calendar\_phantom}\\
\addlinespace
