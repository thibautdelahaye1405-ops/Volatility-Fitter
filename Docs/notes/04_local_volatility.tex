...
optimizations that cut the cold fit $\sim3$--$6\times$ (from $\sim$\,86\,s to
$\sim$\,15--30\,s, scaling with grid size), with recalibrations near-instant:
...
The forward map \eqref{eq:dupire} is solved on a uniform spatial grid with step
$\Delta x=0.01$ on $x\in[0,\sim2.5]$ ($\sim250$ nodes) by fully implicit (backward)
Euler in maturity with step $\Delta T=0.01$ (the local variance entering each step
at the new time level $t_{n+1}$), optionally Rannacher-started with a couple of
implicit start-up steps before Crank--Nicolson. (A separate log-strike validator,
\cref{app:atlas}, runs the finer fixed grid.) Each step solves a tridiagonal
$M$-matrix system
\begin{equation}
...
Diagonal dominance makes pivoting unnecessary, so each step \eqref{eq:implicit} is
a no-pivot Thomas march --- the kernel the compiled march (\cref{app:perf})
accelerates $\sim6.5\times$:

\begin{lstlisting}[caption={The no-pivot tridiagonal (Thomas) solve for one implicit
step \eqref{eq:implicit} (distilled from \texttt{models/localvol/affine\_march.py};
matches a dense solve to $10^{-10}$).},label={lst:thomas}]
def thomas(a, b, c, d):            # a: sub-diagonal, b: diagonal, c: super-diagonal
    n = len(d); cp = np.empty(n); dp = np.empty(n)
    cp[0], dp[0] = c[0]/b[0], d[0]/b[0]
    for i in range(1, n):          # forward sweep: eliminate the sub-diagonal
        m = b[i] - a[i]*cp[i-1]
...
measures the residual: a butterfly tolerance on $\partial_{xx}c$
($\gtrsim-10^{-6}$ density) and a calendar tolerance on $\partial_T c$
($<-10^{-9}$ price decrement). The model is gated behind these
...
\item \textbf{Front tie}: a light penalty tying the unconstrained $t=0$ time row
to the first (data-identified) expiry row across strikes, anchoring the front end
where the shortest expiry has little coverage.
\end{itemize}
...
\midrule
\multicolumn{3}{l}{\emph{Hidden (affine-calibration PDE / solver)}}\\ \midrule
affine PDE step & $\Delta x=0.01$ & Spatial step on $x\in[0,\sim2.5]$ ($\sim250$ nodes).\\
affine time step & $\Delta T=0.01$ & Implicit-Euler maturity step.\\
\texttt{rannacher\_steps} & $2$ & Implicit start-up steps before CN (opt-in scheme).\\
short-dated step & $0.3\,\sigma\sqrt\tau$ & Refined $\Delta x$ floor, clamped $[1/400,0.01]$ (\cref{sec:short}).\\
...
\texttt{DEFAULT\_N\_K} & $1201$ & Validator spatial nodes.\\
\texttt{DEFAULT\_DT\_MAX} & $1/400$ & Validator max time step.\\
...
inverse map --- read a local-variance grid off an \emph{already-fitted}
total-variance surface $w(k,T)$ --- is Gatheral's formula, used for grid export and
the graph LV projection (Note~14). The denominator $g$ is the butterfly function:
$g\le0$ marks strike arbitrage in the implied surface, where no positive local
variance can reproduce it, returned \texttt{nan}. Matches production to $10^{-13}$.
...
def dupire_local_variance(k, w, wk, wkk, wt):  # w=w(k,T) with its k, kk, T derivatives
    r = k/w
    g = 1 - r*wk + 0.25*(-0.25 - 1/w + r*r)*wk*wk + 0.5*wkk   # butterfly denominator
    return np.where((w > 0) & (g > 0), wt/np.where(g > 0, g, 1), np.nan)  # nan if g <= 0
\end{lstlisting}
