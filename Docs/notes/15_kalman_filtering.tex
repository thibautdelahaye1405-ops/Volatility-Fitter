...
\input{figures/kalman_tables.tex}}{%
...
has since received its shipped fix (a fit-path surprise gate: a fit-free
ATM probe for the level row, the previous step's realized innovation for
the shape rows); full-regime validation of that gate is the flagged item
before any active-by-default.
\end{abstract}
...
Below one day, $\Delta t$ itself is a modelling choice, surfaced as
\texttt{filterClock}. The default, \texttt{calendar}, counts calendar hours
--- byte-identical to the legacy convention. The opt-in \emph{session}
clock reweights elapsed time so a
trading session carries \texttt{filterSessionShare} ($0.60$) of a day's
variance and a non-trading calendar day carries
\texttt{filterNonTradingWeight} ($0$) --- the filter-side twin of Note~11's
intraday variance clock, same physics, consumed here as the prediction's
$\Delta t$. The motivation is measured, not aesthetic. On the 2026-07
intraday campaign ($936$ measurements; harness
\nolinkurl{backend/backtest/observation_filter_intraday.py}), $30$-minute
steps moved ATM vol by $19.5$ bp while one
overnight \emph{and one whole weekend} both moved it $\approx55$ bp --- no
calendar-proportional $q$ can calibrate all three step types at once (the
best calendar configuration reads $\zeta$ std $1.04/0.53/0.23$ across them,
i.e.\ honest intraday, badly overwide overnight and weekend), while the
session clock at share $0.60$, weight $0$ and $q=90$ bp reads
$0.95/0.89/0.84$ --- jointly calibrated. One deliberate asymmetry: the
\emph{reset} rule of \cref{sec:lifecycle} stays on calendar hours ---
staleness is a statement about data age, not about variance accrual, and a
quiet weekend must still count toward \texttt{filterResetHours}.
...
\cref{prop:nodouble} says it should. The measured weakness was the shock
...
exists, so the original innovation-gated adaptive $Q$ --- overlay-only by
construction --- could not open for it. That extension has since shipped:
the active path prices surprise \emph{before} fitting, gating the level
row on a fit-free ATM probe and the shape rows on the previous step's
realized innovation.  Its evidence is unit-level --- the harness's
synthetic shock perturbs the thinned fit inputs, not the prepared mids
the probe reads, so the existing sweep under-reports it by construction
--- and a full-regime validation is \emph{the} remaining item before any
active-by-default discussion.
...
moves.  The fit-path extension of the gate has shipped (probe-gated level,
innovation-gated shape); the recorded active-mode shock lag
(\KalActShockSpike--\KalActShockHigh\ bp in the v2 sweep) predates it, and
the gate's full-regime validation is still pending. Short-dated nodes ($\le30$ DTE) are dominated by quote and
...
short end remains the filter's weakest bucket, and the per-handle clock
scales are global: the intraday campaign measured a curvature $\zeta$ std
near $6.4$ on short-dated nodes even with the session clock calibrated for
ATM, so \emph{per-maturity handle scales} are the recorded follow-up. The state is per-node: a graph-coupled filter
...
Session clock: $\Delta t$ shapes; calendar default byte-identical &
\cref{sec:prediction} &
\nolinkurl{volfit/calib/intraday_time.py},
\nolinkurl{api/observation_filter.py} &
\nolinkurl{test_observation_filter_app.py} (session-clock shapes)\\
\bottomrule
\end{longtable}
\endgroup
...
a holiday). Always counted in \emph{calendar} hours, whatever the clock.\\
\texttt{filterClock} & \texttt{calendar} & Prediction time base
(\cref{sec:prediction}): \texttt{session} = the intraday session clock,
jointly calibrated on the 0DTE campaign; \texttt{calendar} is byte-identical
to the legacy convention.\\
\texttt{filterSessionShare} & $0.60$ & Session share of a trading day's
variance on the session clock (half-days scale).\\
\texttt{filterNonTradingWeight} & $0.0$ & Day-weight of a non-trading
calendar day on the session clock (a weekend is worth one overnight, as
measured).\\
