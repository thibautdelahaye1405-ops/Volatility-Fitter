...
Measured on the spike fixtures: SPX shock win $0.42\to1.00$ with
...
chased. (iii)~The \texttt{active} mode is in the harness sweep
(\texttt{--modes overlay,active}); in that mode the measurement IS the MAP
solution, so the baseline is the overlay run's raw column. The SPX pilot has
the MAP at $4.7$ bp against $6.1$ bp raw --- edging the overlay posterior
($5.5$ bp) with $\zeta$-std ${\sim}1.1$: fitting quotes and prediction
jointly beats blending them after the fact, as \cref{prop:nodouble} suggests
it should. The cross-asset active sweep rides the next full-regime run.
...
moves. Short-dated nodes ($\le30$ DTE) are dominated by quote and
