...
$\lambda_0=10^{3}$ the base strength, $\Lambda=\texttt{sivWingPenaltyPct}/100$ the
surfaced dial (default $1$; $0$ disables), and the indicator \emph{doubling} the
weight on the put side to match the measured asymmetry. Because
...
(not rejected) on load. The refine stage carries the same optional blocks as the
...
illiquid American ETFs found them \emph{complementary, not redundant}: the de-Am
repair alone cuts the violation about threefold \emph{and lowers} the in-sample fit
error (it removes the arbitraged quotes the model was chasing), whereas the penalty
alone nearly eliminates the violation but at a steep in-sample cost (it must fight
those same quotes); running both attains the penalty's arbitrage removal at roughly
a third of that cost, because the inputs are cleaned before the constraint is
imposed. That is the quantitative case for shipping both on by default.
\end{remark}
...
(hard-capped at two; a persisted larger value is clamped, and the identifiability
bound \eqref{eq:identifiability} may reduce it further). $0$ = pure sigmoid base.\\
