...
$g(k)\ge0$ on the grid is the no-butterfly check. Unlike LQD, MCS is
...
--- and it is itself \emph{verified butterfly-clean} before any fit runs (see
the remark closing this section).
...
hats own the shoulders. Right: the butterfly diagnostic stays non-negative for
\emph{both} curves --- the fitted slice and the synthetic target it chased,
whose own analytic $g$ is asserted positive by the generator.}
\label{fig:components}
\end{figure}
\begin{figure}[H]\centering\phantomsection\label{fig:diag}\end{figure}

\begin{remark}[The target must be clean too --- a fixed figure bug]
\label{rem:targetbug}
A certificate is only as strong as the exercise it certifies. An earlier
revision of this note's generator built a WW target whose \emph{own}
Durrleman function dipped to $g\approx-0.38$ near the shoulders: the
synthetic ``market'' itself carried butterfly arbitrage, so no admissible
slice could both track it closely and honour $g\ge0$, and the figure's claim
was quietly false. The fix re-chose the shoulder amplitudes and widths ---
the shoulder-top curvature scales like $2A/s^2$ and is what drives $w''$,
hence $g$, negative --- and the wing slope, so the target is genuinely
admissible; the generator now computes the target's $g$ from \emph{analytic}
jets (no finite differences, the measurement lesson of Note~02) and asserts
$g>0$ on the plotted grid before any fit runs. Current values: target
...
on dense data), while the base $R=0$ behaves like SVI; the extra cores buy
in-sample error and \emph{give it back out-of-sample} --- the marginal precision
is negative for $R\ge3$. Two measures followed. The amplitude ridge,
...
the violation ($g\ge-0.02$) but at $749$ bp (it must fight those same quotes);
both attain the penalty's arbitrage removal at $225$ bp, because the inputs
are cleaned before the constraint is imposed. One real node makes the
mechanism vivid (EFA in the Aug-2024 vol spike, 11 days to expiry, 22
quotes): neither defence, $75$ bp with worst $g=-116$; repair alone, a tight
$18$ bp but $g=-12$ remains; penalty alone, arbitrage-free but $726$ bp;
both, arbitrage-free at $34$ bp. The fence is expensive only when it must
fight arbitraged inputs --- the repair is what makes it affordable. On liquid
names both are byte-identical no-ops. That is the quantitative case for
shipping both on by default; the census that sited the penalty (64\% put
wing, median worst violation at $z\approx-3.2$) and the cross-model framing
...
WW figure target and fit butterfly-clean ($g>0$ asserted) &
\cref{rem:targetbug} &
\nolinkurl{figures/gen_siv.py} &
runtime assertion in the generator (no separate test)\\
\bottomrule
\end{longtable}
\endgroup
