...
\begin{document}
\maketitle
\input{figures/mcs_corrections_tables.tex}
...
over a clean slice unchanged to solver tolerance.  At the product boundary
the same diagnostic acquires teeth: a dense traded-range certificate gates
readiness and publish for the displayed slice, MCS included
(\cref{sec:noarb}).
...
finite grid $g$ is a \emph{diagnostic}, never a global certificate.  That is
a statement about the whole real line; on the traded range, a dense sampling
of the same functional is exactly what production now enforces as a gate,
and the distinction between the two scopes is drawn at the end of this
section.
...
Beyond the core diagnostic and this penalty, three further layers stand at
increasing distance from the fit.  An always-on advisory measures the
remaining extrapolated-region $g<0$ per fit.  An export-only wing projection
raises exported prices to convexity while leaving the fitted core pinned
(Note~09).  And --- the layer with teeth --- every slice a user sees is
re-measured by the display \emph{certificate}: Durrleman's $g$ from the
analytic jets, sampled at 801 points across the traded strike range with
tolerance $g\ge-10^{-4}$; a failing slice fails readiness, and the publish
gate refuses the surface outright.  The certificate is a statement about the
sampled grid on the traded range, no more --- but within that scope it is
binding, for MCS exactly as for the other families.  What MCS does
\emph{not} yet have is the automatic certified \emph{repair} refit that SVI
runs when its certificate fails; that asymmetry is an open rider, and until
it closes the fitter's defences here are the calibration bounds, the
amplitude ridge, the put-wing regularizer, and the certificate's hard gate
itself.
...
overlay is no longer a prior exception.  The one block MCS does \emph{not}
get is the automatic certified belly repair that SVI runs when the display
certificate fails --- the open rider of \cref{sec:noarb}.
...
spike replay: out-of-sample RMS \mcscorroosbefore$\to$\mcscorroosafter\ bp ---
a gain of a fraction of a basis point --- for roughly the fourfold fit cost
the two harness timings show
($\mcscorrcostrtwo\to\mcscorrcostrthree$\,ms) and worse arbitrage.
...
signed hats can drive variance negative or break convexity; the wing penalty
is a soft, non-guaranteeing fence, and while the traded-range certificate now
hard-gates what a user can see or publish, MCS still lacks the automatic
certified repair SVI carries when that gate fails --- its open rider
...
zero.  Two tolerances now coexist and must not be conflated: the $-0.05$
above is the \emph{wing-repair regression} bar (orders of magnitude better
than the injected violation), while the display \emph{certificate} of
\cref{sec:noarb} demands $g\ge-10^{-4}$ at all 801 traded-range points
before a slice is publishable.  A finite diagnostic grid is not a global
proof.
...
\addlinespace
Traded-range certificate gates readiness and publish for displayed MCS
slices &
\cref{sec:noarb} &
\nolinkurl{models/diagnostics.py::belly_certificate}\newline
\nolinkurl{api/quality.py} /
