...
a grid every Jacobian column and turned a millisecond LQD fit into a measured
$158$-second one, so LQD uses
...
Two conventions keep the identity honest: ``fair strike equals expected total
variance'' is the continuous-monitoring, continuous-path (diffusion) reading
--- jumps and discrete sampling require the usual corrections, outside this
machinery's scope (the same caveat Note~01 attaches to its closed form).
...
onto the vol-space data residuals of SVI/MCS and the vega-normalized residuals of
LQD/LV:
\begin{equation}
...
scale-dependent amount. The same residual form serves the prior-persistence
\emph{var-swap companion} (Note~13), which pulls toward yesterday's fair
strike where today's quotes are silent.
...
to \emph{minutes} --- a $158$-second fit was measured --- hanging the
calibrate loop.
...
as $-2\,\E[X]$ on its existing grid; SVI/MCS keep the replication because
...
\vsagreebp\ vol bps, validating both. Since committee revision R5 the
\emph{derivative} is native too:
$\partial w_{\text{vs}}/\partial\theta
=-2\int(\partial Q/\partial\theta)\,\Lambda(1-\Lambda)\,\dd z$ rides the
analytic Jacobian's existing nodal quantile sensitivities (Note~01), so a
var-swap-targeted LQD fit no longer even falls back to finite differences ---
value and gradient are both closed-form.

\subsection{SVI and MCS: arithmetic curves}

For SVI and Multi-Core Sigmoid (MCS) the total variance $w(k)$ is closed-form arithmetic
...
(The \emph{evaluation} is cheap; the var-swap block still switches these two
fits to the finite-difference Jacobian --- Note~02 --- which is affordable at
five or $6+4R$ parameters in a way it never was for the inversion-heavy
route.)
...
machinery as the value solve. Its virtue is stated carefully: $g(0,1)$ is
determined by the diffusion \emph{around} $x=1$, so it is far less sensitive
to coarsening or truncating the strike grid in the wings than the
$K^{-2}$-weighted replication tail --- but it still runs on a finite domain
with approximate boundary conditions, so ``no truncation dependence'' would
be too absolute (Note~04 states the same scope). It is gated by
\texttt{varSwapMethod} (default \texttt{static}; the source PDE is the
wing-robust alternative a coarsened calibration grid needs; the parametric
models always use the static replication).

A var-swap quote also changes the LV fit's \emph{parameter set}: it frees
the left-wing extrapolation slope $a$ as a bounded calibration variable
(Note~04). The coupling is the economics of the instrument --- the quote
constrains the global deep-put mass, and the freed slope is exactly the
surface's degree of freedom for meeting it (test-locked: freeing $a$
reduces the var-swap error).
...
\emph{per-model cheap var-swap} --- LQD's closed form \eqref{eq:lqd_vs}, SVI/MCS's
...
constraint cost essentially nothing. The model-side error bar now exists
too: the functional posterior band (Note~14) pushes a fitted slice's
three-handle covariance through to a var-swap standard deviation, so a
targeted quote can be read against the fit's own uncertainty rather than as
a bare number.
...
\addlinespace
LQD var-swap rows on the analytic Jacobian (R5) &
\cref{sec:cheap} &
...
Freeing the left slope reduces the var-swap error (LV) &
\cref{sec:cheap} &
...
SVI/MCS (negligible); the LV source PDE is one extra backward solve with analytic
