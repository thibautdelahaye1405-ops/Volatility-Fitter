...
(structural under the R3 chart)\\
Wing-admissible & $\max(\betL,\betR)<2$ strictly & soft screen fenced at
$\beta_{\max}=\svimomleecap$ (structural under the R3 chart)\\
...
--- the weight can be set to zero, or the cap raised to $2$ and beyond,
re-opening the boundary trap of \cref{prop:tail} --- so they are
...
\paragraph{The structural chart (committee revision R3, shipped opt-in).}
...
(\texttt{models/svi\_jw/structural.py}), finite-difference Jacobian for now.
...
despite the FD Jacobian: the penalty kinks it removes are exactly where LM
...
the first flushed out and fixed a lift-saturation bug): precision better or
...
survivorship artifact of its non-converged third).  The structural chart is
...
(equal quotes, production calibrators, default settings): raw SVI misses by
