...
cross-asset path entirely. The solver documented here is the default
\texttt{smooth\_field} operator; the shipped menu also carries an opt-in
pairwise \emph{precision-message} operator and an opt-in layered
dynamic-harmonic pipeline, both held-out-adjudicated and both documented
...
Everything above is the default propagation operator ---
\texttt{smooth\_field}, the increment prior \eqref{eq:Qdelta}, byte-identical
at its defaults to the solver the backtests validated. It is one entry in a
...
certification case \texttt{graph\_precision\_messages}. The companion
...
\texttt{propagationMode} & \texttt{smooth\_field} & The operator menu
(\cref{sec:menu}): \texttt{precision\_messages} / \texttt{hybrid} /
\texttt{layered\_dynamic\_harmonic} opt-ins. The message operator's policy
