...
transported-prior baseline at $+25.6$ to $+37.1$ ATM vol bps, unbiased
(standardized-residual mean $\approx0$) and slightly conservative (std
$0.72$--$0.90$). A traceability table ties every claim to its module and test.
\end{abstract}
...
$\approx0$ on this pilot --- for two \emph{measured} reasons, not a verdict
against the method. The same-name transported prior is already an excellent
predictor at very high baseline precision (a $96$ bp SPX innovation moves a
fully-dark AAPL node by $0.01$ bp), and the 8-asset capture is structurally
starved of the same-sector clusters and sector ETFs that light the cross-asset
path. The follow-ups --- a 25-asset capture, and a lower dark-node baseline
precision --- are designed to give that path a fair test.
\end{itemize}

\begin{figure}[H]
\centering
\includegraphics[width=0.95\textwidth]{fig_graph_backtest.pdf}
\caption{The leave-one-out verdict. Left: skill over the transported prior for
the headline use case, bracketed by the two transport regimes --- the graph adds
$+25.6$ to $+37.1$ ATM vol bps. Right: the standardized-residual std is below
one in both regimes: the posterior bands are slightly conservative, never
overconfident.}
\label{fig:backtest}
\end{figure}
