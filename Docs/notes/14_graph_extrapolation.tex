...
cross-asset path entirely. The solver documented here is the
\texttt{smooth\_field} operator --- the wire default, byte-identity anchor
and explicit rollback; the desk-facing options default has been the
pairwise \emph{precision-message} operator since 2026-07-27, and the menu
also carries an opt-in layered
dynamic-harmonic pipeline --- all held-out-adjudicated and the richer two
documented
from scratch in this note's companion editions --- the menu and its recorded
verdicts are \cref{sec:menu}. A traceability table ties every claim to its
module and test.
\end{abstract}
...
\begin{remark}[Active observation selection]
\label{rem:active-selection}
The covariance form answers a second question for free: \emph{which dark node
is worth quoting next?} Observing candidate $c$ with precision $r$ is a
rank-one Schur update of the posterior covariance,
\[
K' \;=\; K^+ \;-\; \frac{K^+ e_c\, e_c^{\!\top} K^+}{K^+_{cc}+1/r},
\qquad\text{so}\qquad
G(c)\;=\;\sum_i w_i\,\frac{(K^+_{ic})^2}{K^+_{cc}+1/r_c}
\]
is the exposure-weighted total variance a single quote at $c$ removes ---
closed form, no refit. The candidate columns $K^+_{\cdot c}$ come from the
same identity the solver already uses,
$K^+_{\cdot c}=K^-_{\cdot c}-C\,S_y^{-1}(C_{c\cdot})^{\!\top}$ with
$C=K^-_{\cdot S}$ and $S_y$ stored by the update, so scoring the whole
candidate set costs one $m\times m$ solve
(\texttt{volfit/graph/select.py}, locked exact against a brute-force re-solve).
The idio floor of the calibrated-band work is deliberately \emph{not} folded
into cross-node gains: idio variance is precisely what observing other nodes
cannot remove --- quoting the node itself does.
\end{remark}
...
becomes the projection target of an affine LV calibration. The credible band is
the \emph{functional posterior} of the slice (delta method on the slice map,
\texttt{volfit/models/lqd/band.py}): the full $3\times3$ handle covariance
$\Sigma$ --- the three marginal posterior variances here; the filter's full
state covariance in Note~15's overlay --- is pushed through the retarget map by
central differences along the chart's primary directions, so at every strike
$\mathrm{Var}[\sigma(k)]=g(k)^{\!\top}\Sigma\,g(k)$ with
$g(k)=\partial\sigma(k)/\partial h$. Skew and curvature uncertainty widen the
wings the way a level-only band cannot, and the same six perturbed slices price
var-swap and tail-mass standard deviations for the payload. Band-only by
construction (a request flag restores the legacy level band exactly), validated
against a Monte-Carlo pushforward of the same posterior
(\texttt{test\_functional\_band.py}). Where a dark node does
...
legacy disconnected topology (test-locked). The incident is certification
case \texttt{graph\_dark\_disconnection}.
...
carry shared repricing. \textbf{(iii)} The one dishonest cell in the pack was
the calm regime's dark-name bands ($\operatorname{std}\zeta\approx1.9$ in the
table above, overconfident). That cell is now closed at the ATM level by the
\emph{idiosyncratic band floor} (\texttt{volfit/graph/idio.py}, on by default
in production): the dark ATM band variance is floored at
$\lambda\,\sigma_I^2$ ($\lambda=0.30$), where $\sigma_I$ is a strictly causal,
shrunk EWMA-RMS (half-life 5 trading days, shrunk toward the cross-sectional
pool) of the node's own past lit-node innovations against the transported
prior. Because a dark node's baseline precision enters only its band variance
--- never the posterior mean --- the floor is mean-invariant by construction,
so the skill column above is unchanged; validated offline on the stored
benchmark rows, the two calm cells moved
$\operatorname{std}\zeta\;1.91\to1.02$ and $1.85\to1.03$ with the stressed
cells intact (certification case \texttt{dark\_band\_honesty}). Widening the
\emph{skew and curvature} bands in idiosyncratic tape remains the open
residual.

%======================================================================
\section{The operator menu, and how its defaults were chosen}
\label{sec:menu}

Everything above is the \texttt{smooth\_field} operator ---
the increment prior \eqref{eq:Qdelta}, byte-identical
at its defaults to the solver the backtests validated; the wire default on
a bare solve request, and the explicit rollback. It is one entry in a
shipped menu (\texttt{propagationMode}), and the menu's history is worth
recording because every default in it was set by held-out adjudication, not
preference.

\texttt{precision\_messages} replaces the smoothness field by pairwise
\emph{relation factors} --- calendar ladders and index$\to$name pairs whose
per-edge precisions are first-class policy objects with their own editor,
live preflight and a draft/active lifecycle. Its golden acceptance contracts
--- full transmission, competing signals, dead-informer zero dilution,
shrunk-mode transfer with fixed-$\kappa$ corroboration,
baseline-uncertainty-enters-once --- are locked three times over: against a
brute-force Gaussian reference, through the production assembly, and through
the HTTP API; the configuration lifecycle (draft/active envelope, legacy-blob
migration, corrupt-blob tolerance) round-trips under test. All of it is
certification case \texttt{graph\_precision\_messages}.  Its own
pre-registered \emph{daily} gate did not clear (an RMS tie with narrow
bands, recorded verbatim in the findings file), yet on 2026-07-27 the
desk-facing \emph{options} default flipped to this operator --- on the
intraday async replay, where the smoothness field measures nearly inert
(its zero-innovation anchor at day-scale stiffness shrinks intraday
innovations to nothing) while the messages carry the signal at $65.8$ bp
with honest bands; the wire default, replay and every byte-identity lock
keep \texttt{smooth\_field}.  The companion
\emph{messages} edition of this note documents the operator from scratch;
the \emph{three priors} edition tells all three modes as one story ---
and since 2026-07-27 the lecture editions are the prevailing notes, with
this numbered series kept as reference.

\texttt{layered\_dynamic\_harmonic} adds temporal, directed and harmonic
structure on top of the messages, with a persistent residual store (SQLite,
restored at application startup, corrupt-tolerant). Its adoption status is a
case study in recorded verdicts. The pre-registered \emph{daily} campaign
returned a negative: residual memory is monotone against the half-life at
the one-day horizon (optimum $H\to0$), with a layered spatial edge in
stressed regimes but a calm-regime cost --- verdict RECORD, HOLD. The
decisive \emph{intraday} follow-up (asynchronous replay over an ETF
triangle) then split the question properly: the residual \emph{mechanism} is
real in-session --- an interior optimum near $H^*\approx0.1$ day ($73.7$ bp
against the memoryless arm's $80.6$), genuine OU persistence, calibrated
$\zeta\approx0.8$ --- but the plain precision-message carrier read $65.8$
bp and beat \emph{every} layered arm: the bottleneck is the spatial
carrier, not the memory, and the residual's value dies inside the session.
Layered therefore stays opt-in; the recorded follow-up is a names campaign
(index $\to$ single names). One invariant binds all three modes to Note~13's
provenance rules: \emph{graph output is never prior input} --- only lit,
actually-calibrated nodes enter a prior snapshot, so the propagation loop
cannot echo. And even the in-app autotune obeys the same discipline: it is
the production leave-one-out harness scoring \texttt{etaScale} on real
hold-outs (\cref{app:perf}), not a synthetic sandbox --- the sandbox solver
that once served this purpose has been deleted outright.
...
Functional posterior band $=$ delta-method pushforward of $\Sigma$; matches
Monte-Carlo; band-only toggle &
\cref{sec:posterior} (reconstruction remark) &
\nolinkurl{volfit/models/lqd/band.py}, \nolinkurl{api/graph_band.py} &
\nolinkurl{test_functional_band.py}, \nolinkurl{test_functional_band_api.py}\\
Observation-selection gain closed-form $=$ brute-force re-solve &
\cref{rem:active-selection} &
\nolinkurl{volfit/graph/select.py}, \nolinkurl{api/graph_select.py} &
\nolinkurl{test_graph_select.py::test_closed_form_equals_refit}\\
...
\addlinespace
Message operator: goldens locked vs brute force, production, and the HTTP
API; config lifecycle round-trips (cert.\
\texttt{graph\_precision\_messages}) &
\cref{sec:menu} &
\nolinkurl{volfit/graph/message.py} &
\nolinkurl{test_graph_message_golden.py},
\nolinkurl{test_graph_message_api_golden.py},
\nolinkurl{test_graph_config_migrations.py}\\
Autotune is the production LOO; guards return descriptive 400s &
\cref{app:perf} &
\nolinkurl{api/graph_backtest.py}, \nolinkurl{api/graph_params.py} &
\nolinkurl{test_api_graph.py} (grid/argmin $+$ three 400s)\\
Layered residual store persists and restores; prior-save guard (echo-free) &
\cref{sec:menu} &
\nolinkurl{api/graph_dynamic.py}, \nolinkurl{api/priors.py} &
...
\texttt{propagationMode} & \texttt{smooth\_field} & The operator menu on
the wire
(\cref{sec:menu}): \texttt{precision\_messages} / \texttt{hybrid} /
\texttt{layered\_dynamic\_harmonic}.  The \emph{options} key
\texttt{graphPropagationMode} seeds \texttt{precision\_messages} since
2026-07-27. The message operator's policy
knobs (amplitudes, precision families, calendar policy, message edges) ride
the request/policy envelope; the \emph{messages} edition carries their
atlas.\\
\midrule
...
strictly additive.
\item \textbf{$\eta$ autotune $=$ the production LOO.}
\texttt{POST /graph/autotune} takes the \emph{production} request body and
scores \texttt{etaScale} by the same leave-one-out rule as the backtest ---
calibrated, validation-clean hold-outs through the production solve --- over
the seven-point grid $\{0.1,0.25,0.5,1,2,4,10\}$
(\texttt{AUTOTUNE\_ETA\_GRID}, \nolinkurl{api/graph_params.py}). Guards
return descriptive 400s rather than nonsense tunes: a
non-\texttt{smooth\_field} mode (no $\eta$ knob), active what-if pulses
(hypothesis-firm on every solve, so LOO cannot hold them out), or fewer than
two candidates. The $O(7\,n_{\text{obs}}N^3)$ cost is what the sparse path
targets.
\end{enumerate}
