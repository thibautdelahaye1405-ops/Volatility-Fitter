...
chart with its market-package shape controls, and the calibration objective
with its soft tail barrier and its three optimization charts --- the default
\emph{logistic} chart maps the admissibility wall to infinity, so the
optimizer roams an unconstrained space that covers exactly the admissible
slices. A dedicated section derives the model's \emph{analytic residual
Jacobian} --- a clean cancellation identity that removes the
finite-difference quadrature rebuild, covers every production residual block
except the prior terms, and is, to our knowledge, the original engineering
contribution of this implementation. Every figure and table is generated fresh by the production
...
guarded by a smooth barrier before the wall --- and, in the default logistic
optimization chart, the wall is not even reachable: every unconstrained
iterate is an admissible slice (\cref{sec:charts}).
...
One cross-series remark. The application separately runs a dense-grid belly
\emph{butterfly certificate} on every displayed slice, whatever its model,
and hard-blocks publish on failure (Note~02 --- the penalty-fenced overlay
families genuinely need it). For an LQD slice the certificate is a measured
confirmation of \cref{prop:nobutterfly}, not a constraint the optimizer ever
feels.
...
\subsection{Shape controls in market vocabulary}
\label{sec:packages}

The chart's kernel basis $V$ is unique only up to rotation: ``shape
direction~1'' can rotate or flip between calibrations --- a sound interface
for one-session sculpting, not a persistent trader control. The stable
vocabulary is the market's own quote packages. For the package vector
$P(\theta)=(\text{RR25},\text{BF25},\text{RR10},\text{BF10},\text{VarSwap})$,
evaluated on the slice's own smile, restrict its Jacobian to $\ker J$ and take
the least-norm kernel combination that moves \emph{one} package by one unit
while the handles stay put to first order,
\begin{equation}
\xi_i=\argmin_{\xi}\;|\xi|
\quad\text{s.t.}\quad
\Big(\tfrac{\partial P}{\partial\theta}\,V\Big)\xi=e_i,
\end{equation}
so $V\xi_i$ is ``one unit of package $i$, handles fixed, minimal shape
motion'' (\texttt{models/lqd/packages.py}). The accompanying
\emph{cross-talk matrix} prints how much each package direction moves the
\emph{other} packages --- the honest statement of how independent the
controls really are on a given slice: they are exactly independent only to
first order and only at full kernel rank, and the rank and SVD condition are
reported rather than assumed. Orthogonal to this, the chart's notion of
``least norm'' is itself a choice: passing the Gauss--Newton (Fisher) metric
$G=J_r\T J_r$ of the converged fit re-prices moves by \emph{fit impact} ---
the cheapest handle move becomes the one that disturbs the fitted quotes
least, which is the economically defensible reading. The Euclidean metric
remains the default (byte-identical to the historical chart); promoting the
GN metric to default is an open decision, not a shipped one.
...
\end{equation}
as a bound-free nonlinear least-squares problem (scipy \texttt{trf}, in the
active optimization chart of \cref{sec:charts}). The
...
\subsection{The optimization charts}
\label{sec:charts}

The minimization variable need not be $\theta$ itself. Production offers
three \emph{charts} over the same model family
(\texttt{lqdCoords}; \texttt{models/lqd/charts.py}). A chart changes the
optimizer's trust-region geometry, never the objective, so the fitted optimum
is chart-independent to solver tolerance and the canonical storage and wire
format stay $\theta=(L,R,a_2,\dots,a_N)$ everywhere:
\begin{itemize}[leftmargin=1.4em]
\item \texttt{lr} --- the historical identity chart: optimize $\theta$
itself.
\item \texttt{endpoint} --- $\phi=(\log A_L,\log A_R,a_2,\dots,a_N)$, an
exact unit-determinant \emph{linear} map: holding the first two coordinates
fixed while moving a body mode automatically counter-adjusts $(L,R)$ to keep
both endpoint scales, so the coefficients that fit acute central convexity
can no longer mechanically drag the asymptotic wings while the solver moves.
\item \texttt{logistic} (the default) --- the endpoint chart with the right
tail pushed through the logistic, $A_R=\expit(\rho)$. The wall $A_R=1$ maps
to $\rho=+\infty$: \emph{every} finite iterate is an admissible slice, so
the chart covers exactly the admissible set and the least-squares problem is
genuinely unconstrained. Near the wall the chart compresses
($\dd\log A_R/\dd\rho=1-A_R\to0$), damping trust-region steps precisely
where the raw charts need the soft barrier; far below the wall
$\rho\approx\log A_R$ and the two charts agree to first order.
\end{itemize}
The analytic Jacobian of \cref{sec:jacobian} rides along by the chain rule
(the chart maps are linear, plus one diagonal logistic factor). One numerical
footnote keeps the claim honest: the logistic removes the \emph{mathematical}
wall, not the floating-point one --- beyond $\rho\approx36$, $A_R$ rounds to
exactly $1.0$ in double precision, so the \texttt{EPS\_AR} buffer remains the
hard numerical guard and the barrier \eqref{eq:barrier}, part of the
objective in every chart, keeps the optimizer clear of that region.
...
\cref{sec:jacobian} covers every block except the prior-anchor and operator
rows --- the market and prior var-swap rows ride the same sensitivity pass
since committee revision R5 --- so only a prior-anchor or operator block
switches the whole fit to scipy's finite-difference Jacobian (correct, merely
not accelerated).
...
& normalized price at fixed strike, support-confined and tapered
(\cref{sec:calendar}) & analytic\\
...
turn a millisecond fit into minutes. The row also stays on the analytic
Jacobian path (committee revision R5):
$\partial w_{\text{vs}}/\partial\theta
=-2\int(\partial Q/\partial\theta)\,\Lambda(1-\Lambda)\,\dd z$ rides the
nodal quantile sensitivities the pass of \cref{sec:jacobian} already owns, so
the historical gate that sent every var-swap-enabled fit to finite
differences is lifted. Second, the identity prices the
...
analytic Jacobian of the residual stack --- the mid or band data block,
ridge, calendar slack, barrier and var-swap rows of \cref{tab:stack} --- in
\emph{one} quadrature pass; only a fit with a prior-anchor or operator block
active falls back to finite differences. The key is a cancellation identity,
and it is the cleanest piece of mathematics in the implementation.
...
the $a$-columns, the confined calendar rows \eqref{eq:cal_rows} are
$\sqrt{w_{\text{cal}}}\,\mathrm{taper}_m\,(-\dd C/\dd\theta)$ at the active
strikes (the legacy quantile form differentiates as $-\dd A/\dd\theta$ at its
active $z$-nodes), the var-swap rows ride
$\partial w_{\text{vs}}/\partial\theta
=-2\int(\partial Q/\partial\theta)\,\Lambda(1-\Lambda)\,\dd z$, and the
barrier row is $\expit\!\big(s(A_R-c)\big)\,s\,\partial A_R/\partial\theta$.

\subsection{Scope and measured speed-up}

The analytic Jacobian covers every configuration without prior-anchor or
operator rows; committee revision R5 lifted the historical var-swap gate (the
market and prior var-swap rows ride the same sensitivity pass), leaving only
the prior terms on scipy's two-point finite difference --- correct, merely
not accelerated. \Cref{tab:timing} reports a fresh comparison:
...
\eqref{eq:asset_share} read at quantile level, so it is already on the grid ---
and that convenience turned out to be a trap worth documenting.

\begin{example}[title={Case file: the acute weekly that bulldozed the ladder}]
\textbf{Setup.} An acutely convex short-dated slice (an event week) at the
front of an otherwise consistent ladder, surface coupling on --- the
certification reproduction. \textbf{Failure mode.} Every longer-dated fit
came off its quotes: the far expiry fitted $10.6$ vol bp free and $1095$ vol
bp under the coupling. \textbf{Diagnosis.} The floor was sampled in
$G$-space on the full grid. $G(\alpha)$ integrates the \emph{whole upper
tail}, so a comparison at any $\alpha$ leaks the near slice's extrapolated
wing into the constraint --- the confinement principle of Note~09 violated
in quantile coordinates. Nor is a $z$-windowed $G$ floor a fix: it removes
only about half the drag, because the integral inside the window still
carries the tail. \textbf{Fix.} Enforcement moved to \emph{price space at
fixed strike}, confined to the common quote support (below).
\textbf{Verdict.} The far fit returned to $10.6$ vol bp with the identified
violation still repaired; the certification case
\texttt{symmetric\_calendar} locks the reproduction.
\end{example}

Production therefore enforces the ordering as \emph{support-confined price
rows}: on a tapered strike grid $k_m$ spanning the two slices' common quote
support, the later slice carries hinge rows
\begin{equation}
\sqrt{w_{\text{cal}}}\;\mathrm{taper}_m\,
\big(C_{\text{near}}(k_m)-C_{\text{far}}(k_m)\big)^{+},
\label{eq:cal_rows}
\end{equation}
with $w_{\text{cal}}=$ \texttt{calendarWeight}
(\texttt{calib/calendar.py::confined\_calendar\_floor}); the taper fades the
active set smoothly as quoted spans shift between refits, and the rows sit on
the analytic-Jacobian path like every other core block. The full-grid
quantile floor survives as a legacy threading form and a diagnostic, not as
the production constraint.

Two orchestrations consume these rows (\texttt{surfaceSolver}). The
\emph{sequential} build --- nearest-to-farthest, each new slice fitted
against its predecessor's floor --- is the historical construction, kept as
an explicit option; its defect is traversal-order bias: the first slice is
immutable and every later slice absorbs its errors. The production default
is the \emph{symmetric} solver (\texttt{calib/symmetric.py}): fit every
expiry \emph{independently}; \emph{screen} each adjacent interface for an
\emph{identified} violation --- normalized-call ordering on the common quote
support, vega-normalized so the number reads as a vol gap; then jointly
Gauss--Newton-repair only the \emph{violation-connected components},
stacking each member slice's own standalone residual blocks plus the tapered
interface rows \eqref{eq:cal_rows}. Because every slice keeps its own quote
weights, the correction is allocated by information: a liquid slice barely
moves, an unsupported acute tail absorbs it. A clean ladder is \emph{exactly}
its independent fits, and if an interface stays violated after weight
escalation the remainder is \emph{irreducible slack} --- genuinely
inconsistent inputs --- reported per interface, never silently flattened
into the rest of the ladder.
...
are \emph{measured and reported} rather than assumed away --- since committee
revision R5 in desk units too: the quality report prints the worst identified
violation together with the strike where selling the near call against the
far one pays most, converted to currency per contract, ticks and fraction of
the quoted spread (\texttt{calendar\_violation\_argmax}). Note~10 develops
the model-agnostic calendar machinery; here we only record that for LQD the
natively \emph{measured} object is the asset share $A(z)$, while the
\emph{enforced} object is the confined price row \eqref{eq:cal_rows}.
...
\item \textbf{The logistic admissibility chart} (\cref{sec:charts}).
Reparametrizing the right tail through $A_R=\expit(\rho)$ maps the
integrability wall to infinity, so the calibrator becomes a genuinely
unconstrained least-squares problem over \emph{exactly} the admissible
slices --- with trust-region steps automatically compressed where the wall
used to be.
\end{enumerate}
...
\addlinespace
Package controls move one package with handles fixed; GN metric prices moves
by fit impact &
\cref{sec:packages} &
\nolinkurl{models/lqd/packages.py}\newline
\nolinkurl{test_lqd_r5_controls.py::test_package_controls_move_their_package_and_not_the_handles}\newline
\nolinkurl{::test_gauss_newton_chart_is_a_right_inverse_with_less_fit_impact}\\
\addlinespace
Optimization charts: body modes endpoint-neutral; logistic cannot reach the
wall; same optimum in every chart &
\cref{sec:charts} &
\nolinkurl{models/lqd/charts.py}\newline
\nolinkurl{test_lqd_endpoint_coords.py::test_body_modes_are_endpoint_neutral}\newline
...
\addlinespace
Var-swap rows on the analytic path; var-swap fit agrees with FD &
\cref{sec:varswap} &
\nolinkurl{models/lqd/jacobian.py}\newline
\nolinkurl{test_lqd_jacobian.py::test_jacobian_matches_fd_varswap_rows}\newline
\nolinkurl{::test_varswap_fit_ungated_and_agrees_with_fd}\\
\addlinespace
Calendar floor confined to price space on common support; symmetric repair
leaves clean ladders untouched; desk-unit violation print &
\cref{sec:calendar} &
\nolinkurl{calib/symmetric.py}\newline
\nolinkurl{test_calendar_confinement.py}\newline
\nolinkurl{test_symmetric_surface.py}\newline
\nolinkurl{test_lqd_r5_controls.py::test_calendar_argmax_names_the_cheapest_trade}\\
\addlinespace
...
\midrule
\texttt{nOrder} $N$ & $16$ & Legendre expansion order; $N+1$ parameters.
Schema range $4\le N\le24$ (default raised from $6$, cap from $16$,
2026-07-30): low-vol names whose quoted strip spans $\pm4\sigma$ leave a
$\pm20$bp equioscillating shoulder residual below $N\approx16$. Effective
per-slice order is quote-count-capped (two quotes per parameter,
$N+1\le\text{quotes}/2$, never below $\min(N,6)$) so thin books keep their
historical $N=6$ fits and avoid the over-parameterized latency meander.\\
\texttt{lqdCoords} & \texttt{logistic} & Optimization chart
(\cref{sec:charts}): \texttt{lr} raw $\theta$; \texttt{endpoint}
$(\log A_L,\log A_R,a)$; \texttt{logistic} pushes $A_R$ through
$\expit(\rho)$ so the wall is unreachable. Optimum chart-independent to
solver tolerance.\\
...
structurally more on heavier configurations. Gated only on the prior-anchor
and operator blocks; the var-swap gate was lifted in committee revision R5.
...
def call_price(k, z, u, Q, A):
    ...
    # cash leg in log space: (1 - expit(zk)) rounds to 0 past zk ~ 36.7
    # and steps the far wing; production uses exp(k - logaddexp(0, zk))
    return np.interp(zk, z, A) - np.exp(k - np.logaddexp(0.0, zk))
\end{lstlisting}
