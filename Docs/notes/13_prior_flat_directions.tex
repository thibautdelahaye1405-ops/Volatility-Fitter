...
\input{figures/prior_flat_tables.tex}
...
($\approx0.098$ --- so at any requirement $\pi_{\mathrm{req}}$ of order
one the gate is nearly fully open), then let $s_p\to0$ and confirm the
basket is unidentified regardless of $s_c$.  Repeat for the butterfly with
...
One provenance rule outranks the others, because it guards the loop
this note's own machinery invites.  \emph{Graph output is never prior
input}: only lit nodes with an actual calibration record enter a prior
snapshot, and a dark node's graph-extrapolated surface can never become
a later transported-prior baseline.  The rule matters most where the
temptation is greatest --- \texttt{graph\_only} mode routes the prior
into the graph baseline of Note~14, and without this filter the loop
would echo: today's extrapolation would seed tomorrow's prior, which
seeds the next extrapolation, and the graph would gradually be
listening to itself.  In this note's language, the prior may speak only
where today is silent --- and a surface that was itself inferred from
neighbours has nothing of its own to say.  The invariant is
certificate-locked (the prior-save guard).
...
defaults (the atlas rows \texttt{DEFAULT\_BANDWIDTH} and
\texttt{\_VARSWAP\_PROBE\_STD}).
...
\texttt{priorOperatorCovarianceMode} accepts \texttt{full}, which busts
the fit cache and changes nothing else --- only the diagonal covariance
is ever built or persisted.  \emph{The proxy overstates
...
\texttt{diagonal}/\texttt{full}; \texttt{full} is cache-key only ---
it busts fits and changes nothing else, only the diagonal covariance is
ever persisted.\\
...
\toprule Claim & Object & Code anchor & Test anchor\\ \midrule
\endhead
Echo-free prior save: graph output never becomes prior input &
\cref{sec:transport} &
\nolinkurl{volfit/api/priors.py} &
\nolinkurl{test_graph_dynamic_production.py::test_prior_save_guard_graph_output_never_prior_input}\\
\addlinespace
