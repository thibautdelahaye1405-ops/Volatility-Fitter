...
\input{figures/graph_three_priors_tables.tex}
...
claims to defend, the family's strongest \emph{daily} measured record, and
--- since the 2026-07-27 flip --- the byte-locked rollback and wire default
rather than the holder of the desk-facing one.  \textbf{Messages} asserts relations --- every edge a
...
pre-registered gates whose recorded verdicts are quoted honestly --- both
of them, for the target regime has now been scored.  At the daily
granularity the layered spatial solve carries the family's only measured
edge in the stressed regimes --- exactly where books bleed --- while
day-old residual memory does not pay.  And on the intraday async replay
the mode's central mechanism is \emph{validated}: the residual memory has
an interior optimum near a tenth of a day, with per-bucket skill decaying
exactly as the OU prior predicts --- the daily harness's ``memory never
pays'' was a horizon artifact, as this note argued it would be.  What the
same replay also shows, and this note reports with equal honesty, is that
the message operator's reciprocal carrier outruns every layered arm on
that universe: the recorded decision is the mechanism validated, adoption
held, and a NAMES campaign --- where directedness should finally earn its
cut --- on the books.
\end{abstract}
...
\item Mode and default changes are explicit and evidence-gated: a default
moves only on \emph{recorded} held-out evidence, and the record that moves
it may be a different horizon than the gate that was written --- as on
2026-07-27, when the failed daily gate stayed on the record and the
options default moved on the intraday separation
(\cref{sec:evidence}).  Every mean-semantics behaviour in this note is
...
\gtpidiobeforetwo$\,\to\,$\gtpidioaftertwo).  It held the desk-facing
default for the project's whole history, and \cref{sec:evidence} shows
both the gate that kept it there and the intraday evidence that finally
moved it; it remains the wire default and the byte-locked rollback.
...
regression --- and least commitment is a genuine virtue when nothing has
been asserted; but the auto-lattice means a relation structure is in fact
always configured, and the intraday replay priced the modesty: at
day-scale stiffness the zero-innovation anchor shrinks intraday
innovations to almost nothing, and the field mode measured nearly inert
exactly where the desk works (\cref{sec:evidence}).  It remains the wrong
tool when the desk
...
$\gtpnaivevar/p$, overstating precision by a factor of
$\gtprepeatratio$.  Competing messages
...
exactly the one the intraday replay \emph{did} reward, at an interior
optimum $H\approx0.1$\,d (\cref{sec:evidence}).}
\label{fig:halflife}
\end{figure}
...
propagation segment with \textsf{Layered} explicitly selected for the
what-if (the segment's initial state follows the saved options default,
which is precision messages since the 2026-07-27 flip; this capture
predates it), the configuration and
...
but equally: byte-identity of the wire default and rollback against the
full
legacy suite.
...
\gtpzetabase) --- so \emph{no default change on this evidence}: the daily
gate did not clear, and its failed table stays on the record.  The
default nonetheless moved, on 2026-07-27, and the basis is the intraday
async replay of the next paragraphs, where the two static operators
finally separate: the smooth field measures nearly inert on intraday
innovations (its zero-innovation anchor at day-scale stiffness shrinks
them to almost nothing) while the message carrier reads them at
$65.8$ bp with honest bands.  The flip is the \emph{options} default
only --- the wire default, the byte-identity locks and the harness keep
the smooth field, which is also the explicit rollback.
...
just posted the edge in (i) \emph{with the memory switched off}.  (The
intraday replay below did later price the spatial half on its ETF
triangle, and found it behind the reciprocal message carrier there ---
a counterweight recorded in the next paragraph, with the reason that
universe is the directed cut's worst case.)
...
through directed anchors still an open pass.  The decision: the layered
mode is a
session-level opt-in --- deliberately never seeded from saved options ---
and the daily message-gate \emph{verdict} stands unchanged even though
the default has since moved on other evidence (\cref{sec:evidence},
gate one).

\textbf{The decisive experiment, and its recorded verdict.}  The daily
campaign tests daily granularity, where every held-out node
...
a daily harness cannot see.  The pre-registered asynchronous timestamp
replay over stored intraday history has now run (an ETF triangle, seven
arms over eight sessions, $13{,}356$ scored rows per arm), and its
five-gate table is filled.  The mode's central mechanism is
\emph{validated}: the residual half-life has an \emph{interior} optimum
--- memoryless $80.6$ bp $\to$ $H\approx0.1$\,d at $73.7$ $\to$
never-forget desk at $108.1$ --- and the per-bucket memory skill decays
$18.1\to10.0\to6.4\to2.5$ bp exactly as the OU prior predicts, with
honest bands ($\operatorname{std}\zeta\ 0.80$).  The daily harness's
monotone ``memory never pays'' was a horizon artifact, as this note
argued it would be; carried state \emph{is} worth carrying inside the
session, and the desk's never-forget instinct is measurably wrong at
both horizons --- the dial matters.  One gate failed, and it is the
finding that sets the follow-up: the plain message carrier read the same
sessions at $65.8$ bp, beating every layered arm --- on that universe
the bottleneck is the \emph{spatial} carrier, not the memory (the
directed cut discards target$\to$hub information that reciprocal pooling
keeps, and three near-exchangeable ETFs punish directedness by
construction), and the residual's value lives inside the session, gone
by the next day.  The pre-registered decision rule was applied verbatim:
\emph{record, hold adoption} --- the layered mode stays opt-in, and the
recorded follow-up is the NAMES campaign (a hub with genuinely
asymmetric single names, where a name cannot inform the index and the
cut should finally earn its cost).  But the reader should be clear
about what did \emph{not} wait on that experiment.  Direction, rigidity
...
only candidate; the replay has now said how hard to lean on the memory
(hard within the session, not across the night), the NAMES campaign will
say where direction pays, and neither was ever going to decide whether
the desk needs its language.
...
negative findings (memory at the day horizon; the message RMS tie; the
layered spatial deficit on the intraday triangle) published in the same
breath as the wins --- a failed daily gate kept verbatim while the
default moved on other recorded evidence, an adoption held even as the
mechanism validated --- with the store-purge case
...
adjudication is now two-horizon, and neither horizon sells on RMS}: the
daily verdict recorded the stressed-regime spatial edge and priced day-old
memory at zero; the intraday replay validated the memory mechanism
($H\approx0.1$\,d, in-session) but measured the directed spatial carrier
behind the reciprocal message operator on its exchangeable-ETF universe
--- so the mode's case rests on its stressed-regime daily edge, its audit
semantics, and the NAMES campaign not yet run (all three on the record
above).  \emph{Correlated informers and parents}: the message $q_i$ adds
...
\texttt{propagationMode} (wire) & \texttt{smooth\_field} & The dial on a
bare solve request:
\texttt{smooth\_field} / \texttt{precision\_messages} / \texttt{hybrid}
(config-only) / \texttt{layered\_dynamic\_harmonic}.  Kept at the smooth
field so replay, the byte-identity locks and the harness are untouched;
the layered value is a
session-level opt-in --- saved options never seed it.\\
\texttt{graphPropagationMode} (options) & \texttt{precision\_\allowbreak
messages} & What the application seeds and a desk runs; flipped
2026-07-27 on the intraday separation with the failed daily gate on the
record (\cref{sec:evidence}).  A store that ever saved options keeps its
explicit value until re-saved.\\
\midrule
\multicolumn{3}{l}{\emph{Smooth field (\cref{sec:smooth})}}\\ \midrule
$\eta$, $\kappa$ & autotuned & Directed-smoothness weight (reach);
local-smallness anchor.  Seeded from saved options; the autotune is the
\emph{production leave-one-out} harness over a seven-point $\eta$ grid on
calibrated hold-outs (the old sandbox tuner is deleted), with
descriptive refusals on non-field modes, active what-if pulses, or too
few candidates.\\
...
read from its stored artifact.  The two adjudication verdicts and the
default flip quoted in \cref{sec:evidence} are current as of 2026-07-27,
from the findings records anchored in the traceability table.
\end{enumerate}
...
Campaign records (message gate + the 2026-07-27 flip; layered §16.3
table; intraday adjudication; store-purge post
mortem) &
\cref{sec:evidence} &
