...
The propagation operator is itself a menu --- the default
\texttt{smooth\_field} increment prior, an opt-in pairwise
\emph{precision-message} operator, and an opt-in layered dynamic-harmonic
...
small case study in how defaults are set here. The default,
\texttt{smooth\_field}, is the increment prior of Note~14, byte-identical at
its defaults to the solver the backtests validated. Two richer operators ship
as explicit opt-ins. \texttt{precision\_messages} replaces the smoothness
...
through the HTTP API. The layered dynamic-harmonic pipeline adds temporal,
directed and harmonic structure on top of the messages; its pre-registered A/B
campaign returned a \emph{negative} verdict at the daily horizon (residual
memory monotone against the half-life, optimum $H\to0$), so it stays opt-in
--- the verdict is recorded, not overwritten, an instance of the ``measure
...
from an older design note, the fresh run wins. Beyond the reference series,
most topics also carry one or more standalone \emph{lecture editions} ---
second tellings from a fresh angle (Note~01's percentile ruler, Note~02's two
languages and its moment map, Note~14's three priors for a dark universe,
\dots) --- written to the register codified in
\texttt{Docs/notes/LECTURE\_REWRITE\_GUIDE.md}; the numbered notes remain the
normative reference. This overview is the deliberate
...
\texttt{graphPropagationMode} & \texttt{precision\_messages} &
