...
where not.} LQD and the LV surface are clean by construction; SVI/SIV are
...
calendar coupling, the event clock, priors, betas, the wing penalty: each is
byte-identical to its absence when off --- and that byte-identity is
test-locked, not asserted.
...
(confined, Note~04), the SIV Durrleman penalty (extended, Note~03), the
...
a closed form for LQD, arithmetic for SVI/SIV, a source PDE for LV
...
convicted by per-expiry counts before any model change (Note~04); the SIV
wing penalty sits where the arb census located the violations (Notes~03,
09); the temporal backtests score priors and the graph against honest
held-out baselines (Notes~13, 14).
\end{enumerate}
...
The SIV that invented a put wing (R3$\times$R6 ablation,
...
\texttt{nCores} & $2$ & Multi-Core SIV hat count (schema-clamped at 2) & 03\\
\texttt{sigmoidRidge} & $10^{-2}$ & SIV hat-amplitude ridge & 03\\
\texttt{sivWingPenaltyPct} & $100$ & SIV put-wing Durrleman penalty ($0=$ off) & 03\\
