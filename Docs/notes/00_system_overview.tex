...
One claim organizes the whole series: every guarantee is graded and
\emph{executable} --- structural where a model can carry it, certified at the
publish gate where it cannot, measured everywhere else --- and the production
incidents that taught each design rule are frozen as named certification cases
that re-run on demand. It is the front door: read it once, then any other note
...
arbitrage the honest statement is graded --- structural, certified, measured.
LQD is butterfly-free \emph{by construction} and the LV surface is
structurally arbitrage-clean (positive local variance); SVI and MCS are
\emph{soft-fenced} by sampled penalties, which reduce but do not guarantee
zero residual violation; and the grading no longer stops there: every
\emph{displayed} slice, whatever its model, must pass a dense-grid belly
\emph{butterfly certificate} over its traded range --- Durrleman
$g\ge-10^{-4}$ on an 801-point grid, computed from the model's own
derivatives in a few dozen microseconds --- before it counts as publishable;
an uncertified slice cannot become a mark (Note~02). Residual violation
rates are still measured and reported per fit, never assumed away.
...
through a \emph{directed Bayesian graph} to reconstruct the \emph{dark} nodes.
The propagation operator is itself a menu --- the pairwise
\emph{precision-message} operator (the options default since 2026-07-27),
the \texttt{smooth\_field} increment prior (the wire default and explicit
rollback), and an opt-in layered dynamic-harmonic
pipeline --- with an optional optimal-transport regularization, off by default
($\lambda=0$). The defaults here were chosen by held-out adjudication, not
preference (\cref{sec:graph}).
\end{enumerate}
...
\begin{enumerate}[leftmargin=1.6em]
\item \textbf{Arbitrage-cleanliness is structural where possible, certified
at the gate, measured always.} LQD and the LV surface are clean by
construction; SVI/MCS are fenced by penalties that are exactly zero on
admissible slices --- which bounds but does not eliminate residual violation;
the belly butterfly certificate hard-blocks readiness and publish for
\emph{any} uncertified displayed slice; and the residual rates are reported
by explicit per-fit diagnostics, never assumed away.
...
The propagation operator is versioned and adjudicated, and its history is a
small case study in how defaults are set here.
\texttt{precision\_messages} --- the options default since 2026-07-27 ---
replaces the smoothness
field by pairwise \emph{relation factors} (calendar ladders, index$\to$name
pairs) whose per-edge precisions are first-class policy objects with their own
editor, live preflight and draft/active lifecycle; its golden acceptance
contracts --- full transmission, competing signals, dead-informer zero
dilution, baseline-uncertainty-enters-once --- are locked three times over,
against a brute-force Gaussian reference, through the production assembly, and
through the HTTP API.  Its own pre-registered \emph{daily} gate did not
clear (RMS tie, narrow bands, recorded verbatim); the default moved on the
intraday async replay, where the smoothness field measures nearly inert
while the messages carry the signal --- and \texttt{smooth\_field}, the
increment prior of Note~14 and the solver the backtests validated, stays as
the wire default, the byte-identity anchor and the explicit rollback.
The layered dynamic-harmonic pipeline adds temporal,
directed and harmonic structure on top of the messages; its pre-registered
daily A/B
campaign returned a \emph{negative} verdict at that horizon (residual
memory monotone against the half-life, optimum $H\to0$), and the intraday
replay then \emph{validated} the memory mechanism (interior optimum near a
tenth of a day, OU-shaped decay) while measuring its directed spatial
carrier behind the messages on the ETF triangle --- so it stays opt-in,
both verdicts recorded, not overwritten, an instance of the ``measure
first'' rule of \cref{sec:rules}. Even the in-app autotune is the production
leave-one-out harness, scoring candidate reach $\eta$ on real hold-outs rather
than on a synthetic sandbox.
...
SVI / SVI-JW & Raw and jump-wing SVI; structural chart, buffered Lee cap,
belly certificate + one-shot repair & 02\\
...
Graph extrapolation & Transported prior + directed increments; opt-in
precision messages + layered dynamics; LOO backtest & 14\\
...
(fit quality, arbitrage diagnostics --- including the belly certificate ---
and data-age staleness) and drives the export/reporting path; the same gates
feed the client-facing certification report (\cref{sec:rules}). Data plumbing (providers --- including Massive intraday
...
bring-your-own. Sub-daily expiries (0DTE) are research/replay-grade --- their exit gates
(bitwise replay, hard publish block, warm-slice latency) are
certification-locked, but live 0DTE is a post-hosting, client-entitled
feature.
...
first. The index is not only documentation. Incidents marked $\dagger$ are
registered, essentially verbatim, as named cases of the \emph{certification
pack} (\texttt{backend/backtest/certification.py}): 22 cases across three
dimensions --- market regimes, data failures, model stress --- each pointing
at the pytest locks that regression-guard it forever, so validation and
production share one definition instead of drifting apart. One command
(\texttt{python -m backtest.certification run}) re-executes every lock and
emits a client-facing HTML report. In this series a case file is an
\emph{executable} object: the fastest way to learn the system is to read the
incidents, and the fastest way to trust it is to run them.
...
The Lee cap that sat exactly on the broken boundary ($\beta=2$ passes the
floor and wing screens with zero penalty yet $g(10)=-0.0485$; a live SPY
wing sat at $2.0000$ --- cap now $1.95$)\,$\dagger$ &
a fence must not end on the broken boundary & 02, 09\\
The certified-looking slice with a negative belly (Axel Vogt's classical
parameters pass every wing screen; $\min g\approx-0.033$ mid-range ---
now the 801-point certificate gates publish)\,$\dagger$ &
screens fence the wings; certify the belly & 02\\
The six-day weekly that broke the LV strike grid (fit RMS
$108\to23.5$ vol bp after the fix)\,$\dagger$ &
measure first; per-expiry coverage floors & 04\\
The convex wing that flattened SPY (fit RMS $25.7\to2.6$ vol bp once the
constraint left the quoted range)\,$\dagger$ &
constraints stay off quoted territory & 04\\
The reverted global de-Am repair (ATM smile gap)\,$\dagger$ &
repairs need confined authority & 05\\
The delayed feed that gapped the ATM smile (discount clamp; failed
rate-anchor)\,$\dagger$ & trust the well-identified parameter & 06\\
...
reproduced, then $\to0$ under confinement)\,$\dagger$ &
...
evidence & 13\\
The transient-name topology defect (cross-asset skill exactly $0.000$ bp)\,$\dagger$ &
exact zeros are disconnection, not damping & 14\\
The chart benchmark whose headline favoured the wrong arm (the raw SVI
chart's lower arb rate was a survivorship artifact of its non-converged
third; conditioned on convergence, the structural chart won every regime
median) & condition on convergence before comparing arms & 02\\
The A/B campaign whose arms were byte-identical (a config-hash change
silently purged the residual store on every pair, so all four variants
collapsed to the control) & an ablation must first prove its arms differ & 14\\
...
from an older design note, the fresh run wins. Every topic also carries one
or more standalone \emph{lecture editions} ---
tellings from a fresh angle (Note~01's percentile ruler, Note~02's two
languages and its moment map, Note~14's three priors for a dark universe,
\dots) --- written to the register codified in
\texttt{Docs/notes/LECTURE\_REWRITE\_GUIDE.md}.  As of 2026-07-27 the
lecture editions are the \emph{prevailing} technical notes --- audited
against the code to the same standard --- and the numbered series,
this overview included, is kept as reference.  This overview is the deliberate
...
\texttt{nOrder} & $16$ & LQD Legendre order (range $4$--$24$) & 01\\
...
\texttt{leeSlopeMax} & $1.95$ & SVI Lee wing-slope cap, strictly buffered
under Lee's bound ($2.0$ = the broken boundary, explicit config only) & 02, 09\\
\texttt{sviChart} & \texttt{structural} & SVI optimization chart (every
iterate fence-clean); \texttt{raw} = historical rollback & 02\\
\texttt{bellyRepair} & on & one-shot belly-hinge repair refit when the
certificate fails (kept only if it certifies) & 02\\
...
source PDE & 08\\
\texttt{jointCarry} / \texttt{jointCarryEngageBp} & off / $25$ & joint
borrow/de-Am fixed-point forwards, engaged per expiry above the borrow gate
(below it the parity forward is kept exactly) & 05, 06\\
\texttt{enforceCalendar} & on & ascending-$T$ calendar coupling of Calibrate & 10\\
\texttt{surfaceSolver} & \texttt{symmetric} & calendar coupling: screen +
joint GN repair of violating runs (\texttt{sequential} = legacy one-sided
floor) & 10\\
...
\texttt{graphPropagationMode} & \texttt{precision\_\allowbreak messages} &
propagation-operator seed (flipped 2026-07-27 on the intraday replay
evidence; \texttt{smooth\_field} = explicit rollback, \texttt{hybrid}
config-only; the layered dynamic-harmonic pipeline is a per-request
opt-in) & 14\\
\texttt{idioFloor} & on & dark-node idiosyncratic credible-band floor & 14\\
\texttt{functionalBand} & on & full 3-handle posterior covariance pushed
through the slice map: smile band, var-swap and tail-mass sds (band-only) & 14\\
...
\texttt{filterClock} & \texttt{calendar} & filter time base;
\texttt{session} = intraday session clock (a session carries $0.60$ of a
day's variance, non-trading days $0$) & 15\\
...
\texttt{\_CERT\_POINTS} / \texttt{CERT\_G\_TOL} & $801$ / $10^{-4}$ &
belly-certificate grid and $g$ tolerance & 02\\
