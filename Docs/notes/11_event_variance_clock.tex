...
term-structure interpolation. And the unit trap: the \texttt{EventSpec}
schema docstring (and the legacy clock's) still describe \texttt{weight} in
\emph{years} of extra diffusion time, but production consumes it as
\emph{extra equivalent days} --- confirmed both by the call site (the pairs
feed \eqref{eq:clock} directly) and by the auto-calibrator, which constructs
\texttt{EventSpec(weight=days)}. The day semantics documented here is the
correct, test-locked one; the code comments are the stale party.
\end{remark} Total variance is
