...
\input{figures/wings_lastquote_tables.tex}
...
One boundary fixes this note's scope.  This note owns the strike axis
\emph{beyond} the quotes; its complement on the traded range is Note~02's
belly butterfly certificate, which hard-gates readiness and publish for
any displayed slice.  Together the coverage is complete: certificate with
teeth where the money is, this note's prove/choose/police machinery where
the quotes end.

\begin{invariant}
\begin{enumerate}[leftmargin=1.6em]
\item Every displayed wing is Lee-admissible: $\beta\le2$, structurally
where possible, and by a soft cap \emph{strictly buffered under the
ceiling} otherwise (\cref{sec:choose}).
...
production form --- and the SVI cap's $0.05$ buffer
(\cref{sec:choose}) is the same remark on the other model, bought after
a production SPY wing sat at exactly $2.0000$ (certification case
...
production fits); all far below the operative cap $1.95$, itself strictly
below the ceiling $2$.}
\label{tab:betas}
\begin{tabular}{lrrl}
\toprule
...
the min-variance penalty --- with cap default $1.95$, deliberately
\emph{not} the bound itself.  A hinge whose cap sits exactly at $2$
charges nothing precisely on the broken boundary ray
(\cref{rem:boundary}), and a live SPY expiry was found parked at wing
slope $2.0000$ paying no penalty; the buffered cap closed that trap, and
the incident is locked as certification case \texttt{svi\_lee\_boundary}.
On any admissible smile
...
sits at $b(1+|\rho|)=\wlqsvicap$, far from the fence.  Under the default
structural fit chart (Note~02) the fence is not even reachable: both
wings are strictly below the cap at every iterate by construction, and
the hinge survives as the raw rollback chart's guard.
...
put skew would have to be --- at this $b$ --- before the fence at the
shipped cap $1.95$ activates.  Then repeat the computation at $2$, and
say why the difference is not cosmetic (\cref{rem:boundary}).
...
through the \emph{scalar} wing-slope condition, never pointwise --- and
the same toggle arms the LQD surface's per-interface tail contract under
the symmetric solver: two seam price rows just beyond the span union and
two wing-slope-order rows, so the toggle is not overlay-only.
...
constraints it restores extend there; the export manifest audits it via
\texttt{projectionCalendarWorstBp}, the field that licenses a
price-moving repair at the boundary at all.  Note~10's matching remark
carries the mechanism and the measured lean/cost trade.  All three
phases are certification-locked together as case
\texttt{extrap\_wing\_contracts}; what stays open is doctrine, not
machinery.
\end{remark}
...
$\AR<1$ strictly and the SVI cap is a fence set strictly inside the cone
($1.95$), not a target.  \emph{The
...
measurement (Phase~1), opt-in leaning (Phase~2) and boundary repair at
publish (Phase~3), all three certification-locked
(\texttt{extrap\_wing\_contracts}) --- but not settled
doctrine.
...
\cref{rem:boundary}; cert.\ \texttt{svi\_lee\_boundary}).\\
...
\addlinespace
Buffered SVI cap: the $\beta=2$ boundary trap is closed &
\cref{rem:boundary} &
...
\addlinespace
Three extrapolated-region phases (measure / lean / repair) &
\cref{rem:softwings} &
\nolinkurl{volfit/calib/extrap.py} /
\nolinkurl{volfit/models/projection.py}\newline
\nolinkurl{tests/test_extrap_enforce.py};
\nolinkurl{tests/test_wing_projection.py};
certification case \texttt{extrap\_wing\_contracts}\\
\addlinespace
