...
(\nolinkurl{api/affine_transport.py}) still has no dedicated unit test ---
its three
ingredients (curve rule, quote rule, grid rule) are each locked above, and
an end-to-end integration test
(\nolinkurl{test_affine_lv_surface_transports_without_refit}) drives the
full composite through the API; the unit-level lock remains open.
...
\texttt{dynamicsRegime} & \texttt{sticky\_strike} & Named regime seed for the
spot scenario: \texttt{sticky\_moneyness} / \texttt{sticky\_strike} /
\texttt{sticky\_local\_vol} / \texttt{sticky\_local\_vol\_grid} /
\texttt{custom}.\\
\texttt{ssr} & $2.0$ & Numeric SSR of the \texttt{custom} regime only ---
inert under any named regime.\\
