...
American chains only, each surviving \emph{mid} is de-Americanized. The tick
screen deserves its one sentence: on real-feed chains an OTM quote whose
\emph{bid} is worth no more than three ticks is dropped --- a couple of ticks
carries no volatility information and whipsawed the wings --- with the tick
size persisted alongside the snapshot so replays screen identically
...
carry to a few bps, not the quote to sub-bp. One step deeper exists as an
explicit opt-in: the \emph{joint borrow/de-Am fixed point} of Note~06
(\texttt{jointCarry}) iterates the full loop --- borrow and dividends into
de-Am, de-Am into the parity forward, the forward back into borrow --- to
convergence, and routes the converged $(\fwd,D)$ into the fits per expiry
\emph{only} when the implied borrow clears a materiality gate; below it the
parity forward is kept exactly, so ordinary names are byte-identical even
with the toggle on.
\end{remark}
...
the trusted core, hence invariant~4. The revert and the confined repair's
clean-chain no-op are locked together as certification case
\texttt{deam\_repair\_confinement}.
\end{caution}
...
One boundary hardening came from the field rather than the design. Multiple
listings can quote the \emph{same strike} (weekly and monthly series
overlapping), which puts two identical abscissae into the wing projection ---
and the core-anchor \emph{slope}, a divided difference across the boundary,
divided by zero: the crash killed whole backtest day-pairs before it was
traced. The anchor slope now reads the nearest \emph{strictly distinct}
strike, clean chains are byte-identical, and the case is certification-locked
(\texttt{duplicate\_strikes}).
...
Wing repair: ATM core byte-identical, band-stay, convexity gate; duplicate
strikes do not crash the anchor slope (cert.\ \texttt{duplicate\_strikes}) &
\eqref{eq:bfly_strike}, \eqref{eq:convex_sets} &
\texttt{volfit/calib/convex\_deam.py} & \texttt{tests/test\_convex\_deam.py}
(duplicate-strike lock included)\\
Tick floor drops few-tick OTM quotes; tick size persists with the snapshot
(cert.\ \texttt{tick\_noise\_quotes}) &
--- &
\texttt{volfit/api/quotes.py} &
\texttt{tests/test\_quotes\_deam.py} (tick-floor lock)\\
