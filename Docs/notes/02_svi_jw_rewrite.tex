...
\begin{document}
\maketitle
\input{figures/svi_rewrite_tables.tex}
...
$g=\svirwcounterg$; production therefore measures every displayed slice with
an explicit density certificate rather than trusting the screens.  Finally we
derive the calibration maps --- the historical unconstrained raw chart and the
structural chart that now fits by default, whose coordinates are the certified
quantities themselves --- together with the analytic Jacobian, trace the
optional residual blocks, and run a production
...
cross-expiry order.  Non-negative density on the traded range is instead
measured by a separate certificate that gates every displayed and published
slice (\cref{sec:rw_noarb}).
\item Production fits and stores raw SVI; by shipped default the optimizer
works in the structural chart of \cref{sec:rw_structural}, with the
unconstrained raw chart retained as explicit rollback.  JW remains an
analytical and reading coordinate system: there is no runtime five-handle
entry, bump, or export workflow, though the guarded converter and the
desk-unit ticket that any such workflow must sit behind are shipped and
test-locked (\cref{sec:rw_inverse}).
\end{enumerate}
\end{invariant}
...
without domain guards; it is the fast path for callers that already hold a
regular-domain point.  Inputs strictly outside the skew band typically
generate NaNs, skew-band endpoints create zero denominators, and
$\vmin\ge v$ returns a degenerate or negative width.  Production therefore
also ships a guarded converter, \texttt{jw\_to\_raw\_checked}, which is this
section made executable: it validates every inequality of
\eqref{eq:rw_regular_domain} in the order the theorem establishes them,
rejects the singular stratum \eqref{eq:rw_singular_domain} explicitly
(no numerical rearrangement can select the missing curvature), evaluates the
denominator in the cancellation-resistant form \eqref{eq:rw_D_stable}, and on
failure raises a structured domain error whose machine-readable code names
the violated inequality --- six codes, one per row of the theorem's domain ---
instead of a case-dependent NaN.  Any user-facing five-handle workflow is
required to sit behind it.
\end{caution}

The checked converter answers ``is this JW point a smile?''; a desk also
needs the converse reading ``what does this smile quote as?''.  A shipped
desk-unit ticket converts any fitted slice into the instruments actually
traded: ATM volatility, 25- and 10-delta risk reversals and butterflies with
strikes solved on the model smile via the forward Black delta, the two
\emph{actual} asymptotic wing coefficients (the moment objects of
\cref{sec:rw_noarb}, not the normalized $p,c$), and the var-swap volatility.
A companion bump row re-reads the same ticket after a relative forward error
$\dd F/F$: a wrong forward shifts every log-moneyness in unison and is easily
misread as skew or wing movement, and the ticket shows exactly which desk
quantities it moves.  Together the two functions are the conversion layer any
future JW entry workflow must pass through; both are test-locked now rather
than designed later.
...
Positive/Lee interior & $\wmin>0$, $\betL<2$, $\betR<2$ & hard under the default structural chart ($\wmin>0$, $\max(\betL,\betR)<\svirwleecap$); soft screens under the raw rollback chart\\
...
the second became hard when the default fit chart moved to structural
coordinates (\cref{sec:rw_structural}); the last two require conditions
beyond a single coefficient box, and the third is measured by an explicit
certificate at the display gate.}
...
The default cap deserves one more sentence, because its value is a lesson in
hinge design.  \Cref{prop:rw_g_tails} shows that the boundary ray
$\betR=2$ is itself broken: the far-call boundary condition fails there and
the tail limit of $g$ is zero.  A hinge whose cap sits exactly at $2$ charges
nothing precisely on that broken boundary, so an optimizer pressed by the
data can park a wing at $2.0000$ and pay no penalty --- which a live SPY
expiry was later found doing.  The shipped cap is therefore buffered strictly
inside Lee's bound, at $\beta_{\max}=\svirwleecap$; refitting the incident
slice under the buffer moved the displayed smile by well under a tenth of a
volatility basis point, and the boundary trap is locked as a named
certification case.
...
The story now has a production ending.  A slice like this no longer reaches
a user unflagged: the display certificate below samples $g$ densely on the
traded range and fails it, readiness reports the failure, the publish gate
refuses the surface, and with automatic repair enabled the fitter re-solves
once with a belly hinge and keeps the result only if it certifies.  The
screens still pass --- that is the permanent lesson --- but they are no
...
certified domain.  It instead bounds, certifies on the traded range, repairs,
measures, and projects through six separate layers.

\begin{enumerate}[leftmargin=1.6em]
\item \textbf{Core screens.}  Every fit appends the two minimum/Lee hinge rows
of \eqref{eq:rw_core_rows} below.
\item \textbf{The display certificate.}  Every slice a user sees is
re-measured by a dense sampling of Durrleman's $g$ --- 801 points across the
traded strike range, from the closed-form derivatives --- before it counts as
ready.  A failing slice fails readiness, and the publish gate refuses the
surface outright.  This is a certificate about the sampled grid on the traded
range, stated as such; it is the layer that catches the counterexample below.
\item \textbf{Automatic repair.}  When the certificate fails, the fitter
re-solves once with an explicit belly hinge and keeps the repair only if the
repaired slice certifies.  Clean first fits never see a second solve.  The
repair is currently implemented for SVI only.
...
No item in that list silently upgrades the raw fit to a global certificate on
the whole line.  The exact criterion remains \cref{prop:rw_durrleman}; the
production layers state what they sample or alter, and the certificate scopes
its promise to the traded range where the money is.
...
The optimizer never works directly with a bounded raw vector: it fits in an
unconstrained chart of $\R^5$ and maps back.  Production ships two such
charts.  The historical \emph{raw chart} receives $\theta\in\R^5$ and maps
\begin{equation}
a=\theta_1,\qquad
...
calendar order.  The shipped default is a second, \emph{structural} chart
whose coordinates are the screened quantities themselves; we construct it in
\cref{sec:rw_structural}, after seeing what the residual stack asks any chart
to carry.  The raw chart remains available as explicit configuration --- for
comparability with historical benchmarks, and as rollback.
...
finite-differenced.  Both optimizer charts run this same analytic machinery:
the structural chart of \cref{sec:rw_structural} multiplies the raw-space
Jacobian by a closed-form $5\times5$ chain matrix rather than
re-differentiating anything.
...
On the generated 25-quote case, run in the raw chart for continuity with the
historical benchmarks, a warmed median-of-five run measures
...
same implementation change.  Both panels time the raw chart; the default
structural chart of \cref{sec:rw_structural} is benchmarked separately.}
\label{fig:rw_timing}
\end{figure}

\subsection{The structural chart: fitting in the screened coordinates}
\label{sec:rw_structural}

The raw chart guarantees the hyperbola's structure and nothing else.  The two
quantities the screens police --- the minimum and the wing coefficients ---
are left to soft hinge rows, and nothing prevents a trial, or a converged
fit, from sitting exactly on a hinge; the boundary incident of
\cref{prop:rw_screens_jw} was precisely that.  The alternative is to
parameterize the slice by the policed quantities themselves, so that the
inequalities live in the chart rather than in the penalty.  Production's
default chart describes the slice by
$(\betL,\betR,\kmin,\wmin,\kappa_\star)$ --- the two wing coefficients, the
vertex location, the minimum total variance, and the vertex curvature
$\kappa_\star=w''(\kmin)$ --- and lifts the constrained four:
\begin{equation}
\betL=\beta_{\max}\operatorname{logistic}(\theta_1),\qquad
\betR=\beta_{\max}\operatorname{logistic}(\theta_2),\qquad
\wmin=\operatorname{softplus}(\theta_4),\qquad
\kappa_\star=e^{\theta_5},
\label{eq:rw_structural_lift}
\end{equation}
with the vertex location $\kmin=\theta_3$ entering unlifted, and
$\beta_{\max}=\svirwleecap$ the buffered Lee cap.  Every finite $\theta$
now has strictly Lee-clean wings, a strictly positive minimum, and strict
convexity at the vertex --- by construction, not by penalty.

Recovering the raw coordinates is \cref{prop:rw_geometry} read backwards.
The wings give scale and tilt exactly as in \eqref{eq:rw_inv_wings}; the
vertex curvature, evaluated from \eqref{eq:rw_derivatives} at
$r=s/\sqrt{1-\rho^2}$, is $\kappa_\star=b(1-\rho^2)^{3/2}/s$, which
determines the width; and the vertex equations \eqref{eq:rw_minimum} then
release $m$ and $a$:
\begin{equation}
b=\frac{\betL+\betR}{2},\quad
\rho=\frac{\betR-\betL}{\betR+\betL},\quad
s=\frac{b(1-\rho^2)^{3/2}}{\kappa_\star},\quad
m=\kmin+\frac{s\rho}{\sqrt{1-\rho^2}},\quad
a=\wmin-bs\sqrt{1-\rho^2}.
\label{eq:rw_structural_inverse}
\end{equation}
Two consequences follow immediately.  The two core rows of
\eqref{eq:rw_core_rows} are structurally zero at every iterate, and the
trial-variance floor of the residual never fires, because
$w(k)\ge\wmin>0$ everywhere: the fences become inert bookkeeping.  What the
chart does \emph{not} buy is $g\ge0$ --- the Vogt slice of
\cref{fig:rw_arbitrage} passes wings and minimum with room to spare --- so
the display certificate remains the acceptance authority.

\begin{remark}[Strict in $\R$, not yet strict in float64]
The chart's inequalities are strict for every finite real $\theta$, and both
statements failed in IEEE-754 before the analytic-Jacobian lock exposed them.
A logistic evaluated at a modestly large argument rounds to exactly $1.0$,
parking a wing exactly \emph{at} the cap --- the boundary the chart exists to
keep strict; and with one wing saturated against an ordinary other, the
quotient for $\rho$ rounds to $\pm1$, making $1-\rho^2$ an exact zero, the
width zero, and $m$ a NaN.  Production clips the logistic one ulp inside $1$
and computes $1-\rho^2$ by the exact product identity
$4\betL\betR/(\betL+\betR)^2$.  \emph{A chart proved strict in exact
arithmetic must be re-proved in floating point.}
\end{remark}

The default was not flipped on elegance.  A two-round pre-registered
benchmark over roughly twenty-nine thousand real-chain fits adjudicated the
two charts, and its headline lesson is statistical rather than numerical: the
raw chart's apparently lower aggregate arbitrage rate was a survivorship
artifact.  A third of the raw arm's fits exhausted the evaluation cap before
converging, and those unconverged fits --- rarely arbitrage-flagged, because
they never reached any optimum --- diluted its rate.  Compared converged
against converged, the structural chart is cleaner
(\svirwchartstructarb\% versus \svirwchartrawarb\% genuine-arbitrage
incidence), matches or beats every precision median, never hard-breaks,
exhausts the evaluation cap \svirwchartexhstruct\ times against
\svirwchartexhraw, and fits roughly \svirwchartspeed$\times$ faster.  On
that record the default was ratified; the raw chart stays as explicit
rollback, and the frozen benchmark arm of the backtest harness deliberately
keeps it for continuity.  The laboratory figures of this note run the raw
...
minimum and Lee controls & soft under raw chart & buffered cap $\beta_{\max}=\svirwleecap$; structurally inert under the default chart\\
...
\addlinespace
Guarded converter and desk-unit ticket &
\cref{sec:rw_inverse} &
\nolinkurl{models/svi_jw/svi.py::jw_to_raw_checked}\newline
\nolinkurl{models/svi_jw/desk.py}\newline
\nolinkurl{tests/test_svi_desk_and_guards.py}\\
\addlinespace
Structural chart lift, inert screens, chain Jacobian &
\cref{sec:rw_structural} &
\nolinkurl{models/svi_jw/structural.py}\newline
\nolinkurl{tests/test_svi_structural_chart.py}\newline
\nolinkurl{tests/test_svi_structural_jacobian.py}\\
\addlinespace
Buffered Lee cap and boundary incident &
\cref{prop:rw_screens_jw} &
\nolinkurl{tests/test_svi_lee_boundary.py}\newline
certification case \texttt{svi\_lee\_boundary}\\
\addlinespace
Display certificate gates readiness and publish &
\cref{sec:rw_noarb} &
\nolinkurl{models/diagnostics.py::belly_certificate}\newline
\nolinkurl{tests/test_belly_certificate.py}\newline
certification case \texttt{belly\_certificate}\\
\addlinespace
...
\texttt{sviChart} & \texttt{structural} & Optimizer chart \eqref{eq:rw_structural_lift}; \texttt{raw} is the historical vector \eqref{eq:rw_reparam}, kept for comparability and rollback.\\
\texttt{bellyRepair} & true & One-shot certified repair refit when the display certificate fails; clean fits never see a second solve.\\
\texttt{sviPenaltyWeight} & \svirwpenalty & Residual multiplier $W$ in \eqref{eq:rw_core_rows}; $0$ disables both core screens.\\
\texttt{leeSlopeMax} & \svirwleecap & Buffered cap $\beta_{\max}$, strictly inside Lee's bound of $2$ (the boundary itself is broken); values above the bound are accepted by the schema.\\
...
trial-$w$ floor & $10^{-12}$ & Keeps IV residuals finite on negative-variance trial slices; never fires under the structural chart; not part of \texttt{RawSVI.implied\_vol}.\\
...
inverse is compared with production \texttt{jw\_to\_raw} \emph{and} with the
guarded production converter \texttt{jw\_to\_raw\_checked} on three
regular-domain cases to relative tolerance $2\times10^{-12}$.  The inverse uses
the stable denominator \eqref{eq:rw_D_stable}; the reverse map is exactly the
functional definition \eqref{eq:rw_raw_to_jw}.  Historical note: this listing
predates the guarded production converter, which adopted its domain checks and
stable denominator wholesale --- the reference map was, in effect, promoted to
production.
