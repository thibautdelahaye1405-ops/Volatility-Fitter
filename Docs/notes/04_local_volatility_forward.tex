...
\input{figures/lv_forward_tables.tex}
...
produced it.)  The exposure is certification-locked: a deliberately
coarsened march \emph{must} be revealed by the converged metric
(\texttt{rmsConvergedBp}, reported beside \texttt{arbitrageFree} with every
fit; case \texttt{lv\_operator\_blindness}).  Second, an independent
discretization --- a finer
...
byte (both gates test-locked, and the incident is registered in the named
certification pack as \texttt{weekly\_lv\_resolution}).  The general lesson
is \cref{sec:grid}'s sampling principle wearing production clothes: \emph{a
...
is test-locked and registered as certification case
\texttt{convex\_wing\_tail}.  The lesson echoes Notes~05 and 09: \emph{a
shape constraint must not be imposed where the data already speaks} --- the
same principle at the de-Am input, the MCS output, and here the LV
extrapolation tail.
\end{example}
...
sensitivity-basis budget; above it the exact row-sparse store ($\le8$ slots
per node) takes over (\cref{app:perf}).\\
\midrule
...
per-step hat basis is naturally one dense
$(n_{\text{steps}},n_{\text{interior}},m)$ tensor; on large grids it
outgrew memory and \emph{crashed} rather than slowed.  Above a budget
(\texttt{VOLFIT\_LV\_PHI\_DENSE\_MB}, default $512$\,MB) the build switches
to an exact row-sparse store --- at most eight value/column slots per
interior node, some $30$--$60\times$ smaller --- scattered by a Numba
kernel.  At or below budget the dense path is byte-identical; the
over-budget cases carried no byte-identity obligation, because they used
to die.
...
parallelism \emph{within one fit} (GIL-negative).  Five dead ends
confirming the cost of a single fit is distributed, not localized.
\emph{Across} fits the story differs: the background Calibrate ships slice
fits and per-ticker LV fits to a process pool
(\texttt{VOLFIT\_CALIB\_WORKERS}; serial and pooled runs byte-identical)
