...
| `quoteSync` | bool | **true** | Quote synchronisation (2026-09-24, `volfit.api.quote_sync`): before a calibration every quote of a chain merged from layers of different ages (live ticks beside a REST layer, Bloomberg paints, recorded frames — each tagged with its own spot / stamp by the merge) is de-Americanized and inverted at ITS forward F_i (the app's forward-transport rule), corrected to the chain's forward through the node's last committed fit under the dynamics regime — Δw = w0(x + (R − 1)δ) − w0(x), exactly 0 under sticky-strike — and relabelled at k = ln(K / F_now); total variance is held across the age. Read naively, a 0.1 % spot move mis-reads an ATM 1M IV by ~40 vol bp. Inert (byte-identical) on a chain whose quotes share one spot and stamp. Calibration-affecting. |
| `quoteSyncAgeBpPerSqrtMin` | float 0–100 | 3.6 | IV uncertainty of a quote's age, vol bp per √minute at a 15 % ATM vol (scaled by the node's ATM vol): 3.6 = the SPY intraday campaign's 19.5 bp per 30 min. Widens a stale quote's band by s = a·√age·(σ_ATM/0.15) per side in the band targets; in Mid mode multiplies its weight by 1/(1 + (s/s_half)²) (re-normalized to mean 1). 0 = off. Read while `quoteSync` is on; calibration-affecting. |
...
| `stackCrop` | bool | false | Stacked-IV display crop (2026-09-03, opt-in): each expiry's curve (parametric and LV) is drawn only inside its realistic log-moneyness range — the slice's own [Q(ε), Q(1 − ε)] widened to its quoted range (`volfit.api.crop`; payloads carry the range at fixed tail levels, the frontend interpolates ε). A pricer sampling the fitted distribution never reads the smile beyond with probability 1 − O(ε), so arbitrage-freeness inside the crop is the computational statement. Quotes always drawn. Display-only, no cache invalidation. |
| `stackCropTailProb` | float [1e-12, 1e-2] | 1e-7 | Tail probability ε of the stacked-IV crop, per side (1e-7 ≈ 5.3 Gaussian sd). |
...
| `timeScheme` | `implicit` \| `rannacher` \| `bdf2` | `bdf2` | LV PDE time discretisation (LV operator arc, 2026-09-08). `bdf2` = the second-order, L-stable BDF2 step on the GRADED time grid (geometric from the payoff kink — dt_0 = 1 % of the first mark, dt = min(0.25 t, 0.05, 1.25 dt_last, slab/8) — every expiry AND every vertex row a grid point, ≥ 8 steps per slab, ceiling 0.05 y): ≤ 5 bp of operator error per expiry on every fixture where implicit's per-interval rule left 15–170 bp for the calibration to absorb, at 98–116 steps vs 51–271. Its sensitivity step keeps the implicit kernel's single fused source (one axpy more per level), which is why it, not CN, is the default; the compiled march covers it and Rannacher. `rannacher` (CN after two implicit start-up steps) = opt-in: ≤ 1.6 bp on the same graded grid but ~2× the sensitivity cost, and CN is not monotone (an arbitrage violation appeared on a coarse grid). `implicit` = the first-order byte-identical legacy on the uniform per-interval grid (dt ≤ 0.01, short intervals lifted to 32 steps; every golden lock). Var-swap fits under Rannacher keep implicit. LV-only (affine key). |
| `lvLattice` | `uniform` \| `graded` | `graded` | LV PDE strike lattice (LV operator arc, 2026-09-08). `graded` = each expiry gets its own step over its own support — its traded range widened to ± 6 σ√τ in log-moneyness, at clip(0.15 σ√τ, 1/800, 0.01) — the wings 0.02, the step changing by ≤ 15 % per cell (a Lipschitz envelope of the per-expiry requirements, so the non-uniform second difference keeps its accuracy); x = 1 (the var-swap anchor / ATM row) is a node by construction, the lattice being integrated outward from it. Same near-money resolution, a fraction of the nodes: a SPY surface with a 2-day rung ~1700 → ~400. `uniform` = the legacy: one step for the whole lattice, the SHORTEST rung's 0.15 σ√τ (byte-identical). The converged reprice, the put twin, the display wing march and the Compare tab's twin all ride the fit's lattice. LV-only (affine key). |
...
| `lvSolver` | `trf` \| `gn` | `gn` | Matrix-free Gauss–Newton avoids the trust-region solver's dense SVD (~52% of an eval). Since the active-set step (2026-09-09) it runs the Mid AND the Bid-Ask / Haircut targets with the fast kernel active — 2.5–4× faster than trust-region on the desk fixtures at the same target fit; still falls back to trust-region for var-swap fits, the robust re-solves, or the banded march. Accepted trade-off: GN lands a slightly different local optimum (usually a lower objective; converged-operator error within ~1 bp). |
| `lvXMaxMin` | float [2.5, 10] | 2.5 | FLOOR on the LV CALIBRATION lattice's right edge in x = K/F (x_max = max(1.4 × highest quoted x, floor), closed by the Dirichlet condition C(x_max) = 0). Since 2026-09-02 the displayed right wing rides its own buffered display lattice to k = +1.0 and never inverts inside a Dirichlet boundary layer (`affine_views_ext.display_lattice` / `clean_right_edge`), so this floor only moves the calibration's far boundary — worth raising for high-vol, long-dated names whose true call at 1.4 × the last quote is not negligible (the zero boundary otherwise bends the fitted wing). 2.5 (k ≈ +0.92) = the historical constant, byte-identical. O(n_x) march cost. LV-only (affine key). |
| `densitySmoothWeight` | float [0, 1000] | 1.0 | Density-smoothness penalty μ (2026-09-03): third differences of the lattice call prices (the slope of the Breeden–Litzenberger density) inside each expiry's quoted window ± 2 ATM stds, scaled so a Gaussian slice contributes O(μ) at any maturity / lattice step; the Jacobian rides the marched sensitivities (no extra PDE work, ~1 ms/eval). Targets the vertex-scale local-variance ringing that spikes the density. SPY weekly fixture at μ = 1: converged rms 20.2 → 18.5 bp, nfev 62 → 43. 0 = off (pre-2026-09-03 fit, byte-identical). LV-only (affine key). |
