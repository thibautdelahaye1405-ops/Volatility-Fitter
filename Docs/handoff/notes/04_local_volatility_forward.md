...
The forward map of equation (2) is solved on a strike grid over $[0,x_{\max}]$, $x_{\max}\gtrsim2.5$, default step $\Delta x=0.01$ (refined for short-dated surfaces per "What the grids must resolve"), with Dirichlet boundaries $c(\cdot,0)=1$ and $c(\cdot,x_{\max})=0$, by *fully implicit* Euler steps of size $\Delta\tau=0.01$, the local variance entering at the new time level (the byte-identical legacy; since 2026-09-08 the default march is BDF2 on a graded time grid and the strike lattice is graded per expiry — the closing note of §5.2 "Why not Crank–Nicolson"). On a possibly non-uniform grid with gaps $h^{-}_i=x_i-x_{i-1}$ and $h^{+}_i=x_{i+1}-x_i$, the operator $A$ encoding $\tfrac12\nu x^{2}\partial_{xx}$ has the three-point stencil
...
Nor is the trade-off hypothetical: when the Rannacher/CN variant was evaluated as a speed lever it bought little and still produced a measured coarse-grid arbitrage violation on a test surface, so the default remained fully implicit (Appendix B "Performance notes") — until the following.

> **Update 2026-09-08 (the LV operator arc).** The default march is now **BDF2** — second order and L-stable: its amplification factor vanishes on the stiffest modes, where Crank–Nicolson's tends to $-1$ (the Figure 4 oscillation) — on a **time grid graded from the payoff kink** ($\Delta\tau_0$ = 1 % of the first mark, then $\Delta\tau=\min(0.25\,\tau,\ 0.05,\ 1.25\,\Delta\tau_{\mathrm{last}},\ \mathrm{slab}/8)$, every expiry *and* every vertex row a grid point). Rationale, in two sentences: implicit Euler's kink error at the money is $\sim0.15\,\sigma/N$ for $N$ steps on any front, and the calibration bends $\theta$ to cancel it — the per-interval rule left 15–170 bp of operator error per expiry on the fitted surfaces. BDF2 on the graded grid reads ≤ 5 bp per expiry at 98–116 steps where the legacy rule marched 51–271, and its sensitivity step keeps the implicit kernel's single source (an implicit step plus one axpy per level, where CN's explicit half roughly doubles the work), which is why BDF2 rather than CN became the default. Every scheme is one generic two-level step $(I-\gamma\Delta\tau A^{n+1})U^{n+1}=\alpha U^{n}-\beta U^{n-1}+\varepsilon\Delta\tau A^{n}U^{n}$ — implicit $(1,1,0,0)$, CN $(\tfrac12,1,0,\tfrac12)$, BDF2 with $\omega=\Delta\tau_n/\Delta\tau_{n-1}$: $\gamma=\tfrac{1+\omega}{1+2\omega}$, $\alpha=\tfrac{(1+\omega)^2}{1+2\omega}$, $\beta=\tfrac{\omega^2}{1+2\omega}$, $\varepsilon=0$ — and the implicit factor $I-\gamma\Delta\tau A$ is an $M$-matrix for any $\gamma\in(0,1]$, so Proposition 3's no-pivot solve carries over (the two-level right-hand side does not carry Corollary 1; density positivity stays measured on the output, §5.3, and was clean on every fixture). Crank–Nicolson/Rannacher remains an **opt-in** that now also runs on the compiled march; implicit Euler remains the **byte-identical legacy** on its uniform per-interval grid (every golden lock). The **strike lattice is graded per expiry** (`lvLattice`, default `graded`): each expiry's traded range $\pm6\,\sigma\sqrt\tau$ at its own step $\mathrm{clip}(0.15\,\sigma\sqrt\tau,\,1/800,\,0.01)$, the wings $0.02$, the step changing by at most $15\,\%$ per cell, built outward from $x=1$ (always a node) — a SPY surface with a 2-day rung goes from ~1700 to ~400 nodes at the same near-money resolution. Derivations and the coefficient table: `Docs/piecewise_affine_local_variance_calibration.tex`, eqs. (generic_two_level_step), (bdf2_step), (generic_sensitivity_step) and its "Graded grids" subsection.
...
| `timeScheme` | `bdf2` | Time scheme (2026-09-08): BDF2 on the graded time grid (default — second order, L-stable), opt-in Rannacher/CN (same graded grid, compiled), or the byte-identical implicit-Euler legacy (§5.2 "Why not Crank–Nicolson" and its 2026-09-08 note). LV-only cache key. |
| `lvLattice` | `graded` | PDE strike lattice (2026-09-08): graded per expiry (each expiry's traded range $\pm6\,\sigma\sqrt\tau$ at its own $0.15\,\sigma\sqrt\tau$ step, wings $0.02$, $\le15\,\%$ change per cell, $x=1$ a node) or the legacy `uniform` at the shortest rung's step. LV-only cache key. |
...
| `_DT_MAX` | $0.01$ | Implicit-Euler maturity step ($0.03$ under Rannacher) — the `implicit` legacy grid only. |
| `TIME_GROWTH` / `TIME_FIRST_FRAC` / `TIME_DT_MAX` / `TIME_SLAB_STEPS` | $0.25$ / $0.01$ / $0.05$ / $8$ | Graded time grid (`bdf2` / `rannacher`, `pde_grids.py`): $\Delta\tau\le c\,\tau$ from the kink, first step as a fraction of the first mark, absolute ceiling, minimum steps per slab between marks (expiries ∪ vertex rows). |
| `REGION_SD` / `WING_DX` / `STRIKE_RATIO` | $6$ / $0.02$ / $1.15$ | Graded strike lattice (`lvLattice = graded`): region half-width in ATM standard deviations, the wing step, the maximum step ratio per cell. |
| `BDF2_MAX_RATIO` | $2$ | Step-growth ratio above which a BDF2 march restarts with an implicit Euler step (zero-stable below $1+\sqrt2$); never reached on the graded grid after the first step. |
...
| `_PDE_NT_FIRST_GATE` / `_PDE_NT_SHORT` | $8$ / $32$ | Short-interval time refinement: $32$ steps whenever the flat ceiling would give $<8$ (the `implicit` legacy grid; the graded grid resolves the kink geometrically instead). |
...
7. **Shelved** (documented in the perf roadmap): coarse-grid calibration (biases $\theta$ by up to $\sim26$ vol points — non-viable); the first Numba attempt (wrong loop order, $1.2\times$); Rannacher/CN as a *speed* lever ($\sim1.1\times$ and a measured coarse-$x$ arbitrage, §5.2 "Why not Crank–Nicolson" — superseded 2026-09-08 by BDF2 on the graded time grid as the default, with CN an opt-in that runs compiled; the second-order scheme was adopted for *accuracy*, ≤ 5 bp of operator error per expiry where implicit left 15–170); GN for the non-smooth band objective; thread/process parallelism *within one fit* (GIL-negative). Five dead ends confirming the cost of a single fit is distributed, not localized. *Across* fits the story differs: the background Calibrate ships slice fits and per-ticker LV fits to a process pool (`VOLFIT_CALIB_WORKERS`; serial and pooled runs byte-identical) — parallelism pays between independent solves, not inside one.
