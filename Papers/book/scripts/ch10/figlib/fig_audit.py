"""Figure 10.7 -- the standardized-error test.
...
a starved, a true-scale, and a surprise-widened true-scale prediction
budget.  Panel (a): a window around a jump.  Panel (b): the standardized
error's std against the assumed walk scale -- std(Z) is near one only near
the true scale, and a starved budget is overconfident.  Panel (c): recovery
...
def fig_flt_audit() -> str:
    ...
    m_true_scale, v_true_scale = _run(obs, noise_sd, Q_TRUE, gated=False)
    ...
    band_lo = 1e2 * (m_true_scale[w0:w1] - np.sqrt(v_true_scale[w0:w1]))
    band_hi = 1e2 * (m_true_scale[w0:w1] + np.sqrt(v_true_scale[w0:w1]))
    ax_a.fill_between(days, band_lo, band_hi, color=PALETTE["band"],
                      alpha=0.9, zorder=1)
    ax_a.plot(days, 1e2 * m_true_scale[w0:w1], color=PALETTE["model"], lw=1.4,
              label="true-scale budget")
    ...
    # (b) the test curve: Z std vs the assumed walk scale.
    ...
    figstyle.panel(ax_b, "b", "the standardized-error test")

    # (c) recovery after the jump.
    rec = {
        "starved": (_recovery(truth, m_starved), PALETTE["third"]),
        "true scale": (_recovery(truth, m_true_scale), PALETTE["model"]),
        "true scale + widening": (_recovery(truth, m_gated), PALETTE["alt"]),
    }
    ...
    z_true = _zstd(truth, m_true_scale, v_true_scale)
    z_gated = _zstd(truth, m_gated, v_gated)
    rec_s, rec_h, rec_g = (rec["starved"][0], rec["true scale"][0],
                           rec["true scale + widening"][0])
    ...
    STORE.add("audit", "FiltAuditZstdHonest", num(z_true, 2),
              "std of Z at the true walk scale (30 bp)")
    STORE.add("audit", "FiltAuditZstdGated", num(z_gated, 2),
              "std of Z at the true scale with the surprise widening")
    ...
    STORE.add("audit", "FiltAuditDayZeroHonest", num(rec_h[0], 1),
              "true-scale |error| on the jump day (pts)")
    STORE.add("audit", "FiltAuditDayZeroGate", num(rec_g[0], 2),
              "surprise-widened |error| on the jump day (pts)")
    STORE.add("audit", "FiltAuditDayThreeStarved", num(rec_s[3], 1),
              "starved-budget |error| three days after the jump (pts)")
    return (f"Z std starved {z_starved:.1f} / true-scale {z_true:.2f} / "
            f"widened {z_gated:.2f}; day-0 err {rec_s[0]:.1f}/{rec_h[0]:.1f}/"
            f"{rec_g[0]:.2f} pts")
