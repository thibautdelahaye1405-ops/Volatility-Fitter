"""Shared Matplotlib style for the book's Chapter 7 figure suite.

Identical conventions to Chapters 2-6 (scripts/ch06/figlib/figstyle.py): the
same restrained colorblind-safe palette, serif STIX text at 9 pt, and the
'(a)  ...' left-aligned panel titles.  Chapter 7 keeps observed quotes and
naive-inversion exhibits in the data orange, tree/de-Americanized objects in
the model blue, comparator curves in aqua/violet, and reference geometry in
ink/muted greys.  Magnitude heatmaps use 'cividis' with out-of-scope cells
masked light grey (Chapter 4's precedent).
"""
...
FIG_DIR = Path(__file__).resolve().parents[3] / "figures" / "ch07"

PALETTE = {
    "model": "#2a78d6",  # tree / de-Americanized curves
    "data": "#eb6834",   # quotes, naive-inversion exhibits, measured points
    "alt": "#1baf7a",    # comparator curve (yield model, European leg)
    "third": "#8a5cd6",  # third curve where three coexist
    "ink": "#1f2430",    # reference / truth, emphasis text
    "muted": "#898781",  # de-emphasized geometry, callout text
    "grid": "#e1e0d9",   # hairline gridlines
    "band": "#dfe7f2",   # shaded bands
    "mask": "#d8d6cf",   # out-of-scope heatmap cells
}
...
def save(fig, slug: str) -> Path:
    """Save ``figures/ch07/<slug>.pdf`` (plus an optional PNG preview)."""
    FIG_DIR.mkdir(parents=True, exist_ok=True)
    path = FIG_DIR / f"{slug}.pdf"
    fig.savefig(path)
    png_dir = os.environ.get("CH07_PNG_DIR")
    if png_dir:
...
def whiskers(ax, x, lo, hi, color=None, lw: float = 0.9, alpha: float = 0.85,
    ...
    """Per-quote intervals as thin vertical whiskers (never crosses)."""
