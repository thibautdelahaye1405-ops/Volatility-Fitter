"""Shared Matplotlib style for the book's Chapter 3 figure suite.

Identical conventions to Chapter 2 (scripts/ch02/figlib/figstyle.py): the
same restrained colorblind-safe palette, serif STIX text at 9 pt, and the
'(a)  ...' left-aligned panel titles.  Chapter 3 adds one convention: when
the three families appear together, LQD is drawn in the comparator aqua,
SVI in the model blue, and MCS in the violet accent below, so the family
identity is stable across every comparison figure.
"""
...
FIG_DIR = Path(__file__).resolve().parents[3] / "figures" / "ch03"

PALETTE = {
    "model": "#2a78d6",  # slot 1 blue: SVI (this chapter's primary family)
    "data": "#eb6834",   # slot 2 orange: quotes, targets, observed objects
    "alt": "#1baf7a",    # slot 3 aqua: LQD comparator (direct-labeled)
...
# Family -> color, used by every comparison panel.
...
def save(fig, slug: str) -> Path:
    """Save ``figures/ch03/<slug>.pdf`` and release the figure."""
