"""Shared Matplotlib style for the book's Chapter 11 figure suite.

Identical conventions to Chapters 2-10 (scripts/ch10/figlib/figstyle.py): the
same restrained colorblind-safe palette, serif STIX text at 9 pt, and the
'(a)  ...' left-aligned panel titles.  Chapter 11's role assignment: lit
observations and data-only reads in the data orange; the graph posterior in
the model blue; the transported baseline (ride-the-prior) in aqua; the
failing alternative (the averaging assembly, the overtrusting arm) in
...
FIG_DIR = Path(__file__).resolve().parents[3] / "figures" / "ch11"

PALETTE = {
    "model": "#2a78d6",  # the graph posterior
    "data": "#eb6834",   # lit observations / data-only reads
    "alt": "#1baf7a",    # the transported baseline (ride the prior)
    "third": "#8a5cd6",  # the failing alternative (averaging, overtrust)
    "ink": "#1f2430",    # truth / reference, emphasis text
    "muted": "#898781",  # de-emphasized geometry, callout text
    "grid": "#e1e0d9",   # hairline gridlines
    "band": "#dfe7f2",   # shaded bands (posterior +- 1 sd)
...
def save(fig, slug: str) -> Path:
    """Save ``figures/ch11/<slug>.pdf`` (plus an optional PNG preview)."""
    FIG_DIR.mkdir(parents=True, exist_ok=True)
    path = FIG_DIR / f"{slug}.pdf"
    fig.savefig(path)
    png_dir = os.environ.get("CH11_PNG_DIR")
    if png_dir:
