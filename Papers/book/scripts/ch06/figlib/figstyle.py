"""Shared Matplotlib style for the book's Chapter 6 figure suite.

Identical conventions to Chapters 2-5 (scripts/ch05/figlib/figstyle.py): the
same restrained colorblind-safe palette, serif STIX text at 9 pt, and the
'(a)  ...' left-aligned panel titles.  Chapter 6 keeps observed quotes in the
data orange, fitted lines and estimator curves in the model blue, comparator
estimators in aqua/violet, and reference geometry in ink/muted greys.
"""
...
FIG_DIR = Path(__file__).resolve().parents[3] / "figures" / "ch06"

PALETTE = {
    "model": "#2a78d6",  # fitted lines, primary estimator curves
    "data": "#eb6834",   # quotes, staged failures, measured points
    "alt": "#1baf7a",    # comparator estimator / second curve
    ...
    "band": "#dfe7f2",   # shaded bands (trim band, HTB range)
}
...
def save(fig, slug: str) -> Path:
    """Save ``figures/ch06/<slug>.pdf`` (plus an optional PNG preview)."""
    FIG_DIR.mkdir(parents=True, exist_ok=True)
    path = FIG_DIR / f"{slug}.pdf"
    fig.savefig(path)
    png_dir = os.environ.get("CH06_PNG_DIR")
    if png_dir:
