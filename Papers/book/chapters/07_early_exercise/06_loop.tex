...
mid-2026, which is noise.  Following
\cref{sec:fwdclamp}'s rule, the rate is pinned by a stated
...
conventions the book's figures follow) computed with its own discrete
dividend schedule.  Read that residual gap for what it is: the
distance between two stated dividend conventions
...
a formality; it is where the smile's raw material gets clean.
...
limit.  The resolution is this section's loop with one more unknown
...
and the two passes above suffice.
