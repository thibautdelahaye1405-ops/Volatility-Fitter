...
honest to the extent the model's premium matches the market's.  Inside
...
\subsection{When the root exists, and the honest failure when it
does not}
...
\cref{sec:deamwhere}'s dead zone met in the flesh, and the honest
output is a refusal --- ``this quote determines no volatility'' ---
not a number.  The refusal costs nothing downstream: a quote with no
...
honest, measured size on real data: halving the whole-chain depth from
...
in the model, approximate outside it; disclosed in the contract.
\end{itemize}

We now own the subtraction end to end: a root, its meaning as a
premium removal, its unique-existence theorem, its honest refusal,
and its priced errors.  What we have not yet earned is the tree's
