...
policed.  Conditions that are properties of a \emph{single} curve ---
density positivity above, call convexity --- must be checked
\emph{into} the wings: one law is being asserted there, the condition
needs no second curve to compare against, and abandoning it past the
last quote is exactly how the hat's dent would go unnoticed.
Conditions that \emph{compare two} curves reach exactly as far as the
curves are published.  A calendar inversion at an unquoted strike is
a disagreement between two chosen conventions --- two entries of
\cref{tab:vswings}, possibly from different rows --- but both
conventions are published prices, and an inversion between published
prices is a calendar spread with a negative price.  Chapter~2 draws
the consequence for a model defined on the whole line: the maturity
stack is fitted jointly under the full-line ledger constraint
(\cref{eq:globalledgerconstraint}), with one tail policy stated for
the stack (\cref{eq:tailscalecalendar}), so the wings inherit the
order from that policy rather than from a quote.  A model left
undefined beyond its quotes has nothing to order there; a model that
publishes its wings must order them.  One curve: extend the check.
Two curves: state one tail policy, then extend it too.
