...
\emph{The field-side check.}  The sheet is Chapter~4's whole-surface
SPY calibration, unchanged.  Both of its var-swap routes run on a
dedicated lattice with the calibration lattice's strike step, widened
to strike ratio $12$ so the static replication sees the entire wing
the extrapolated field implies; the put leg is cut off below strike
...
the check reports the pair of reads at the march's own step and at an
eightfold refinement; the gap halves with the step, at the
first-order rate.
