...
\begin{proof}
The proposition reduces to a pointwise identity: for every $y>0$,
\begin{equation}
...
\cref{eq:generalpayoff}, the change of variables under which Chapter~2
...
$|z|e^{-|z|}$ --- Chapter~2's exponential-tail subclass makes the
transport asymptotically linear in $z$ (lighter tail exponents make
it grow slower still), and the logistic weight supplies the
...
checking that they do is a free, end-to-end test of an
implementation.  (Route two costs one \emph{quadrature} --- one
numerical integral --- and the agreement checks below are limited only
by quadrature accuracy.)
...
The field route's check deserves its own sentence, because it teaches
...
schemes' first-order rate, and measuring that convergence is the
check.
...
parameter derivatives, from one quadrature: LQD holds
