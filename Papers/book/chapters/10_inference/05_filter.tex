...
\subsection{The dial nobody may own}

Pause on what \cref{eq:infupdate} does \emph{not} contain.  Every
fitter that carries state through time owns, somewhere, a dial
marked \emph{trust}: how far should this morning's quotes move
yesterday's surface?  The tempting implementation is a slider ---
tuned by hand, per desk, per mood.  In \cref{eq:infupdate} that
dial is the gain, and the gain is not a setting: it is an
\emph{output}, the ratio of two variances.  Every design question
about trust becomes a question about building
$\mathcal{V}^{-}$ and $\mathcal{V}_{\mathrm{obs}}$ honestly ---
which is auditable --- rather than about where to set a slider ---
which is not.  When both variances tell the truth, the dial turns
itself: a noisy morning arrives with a large
$\mathcal{V}_{\mathrm{obs}}$ and is barely admitted; a clean morning
after a quiet night arrives against a small $\mathcal{V}^{-}$...
and the update follows the market.  The gate of
\cref{sec:infgate} and the gain of \cref{eq:infupdate} are the
same philosophy on the two sides of the chapter: the first answers
``\emph{where} did the market not speak?''\ with a dead zone; the
second answers ``the market spoke here --- \emph{how noisy was its
voice?}''\ with a ratio that is never zero and never one.
...
built by the machinery of the next section, come out as standard
...
gains.  Nobody set anything: the gains are ratios of stated
variances.}
\label{fig:infupdate}
\end{figure}
...
curvature is the story: prediction ten times tighter than the
observation, gain
...
bars: trust, computed per handle, unequal because the evidence is
unequal.
...
discarded cross-handle information is real, and this is the
...
$\mathcal{V}^{-}$ grow honestly between snapshots?
