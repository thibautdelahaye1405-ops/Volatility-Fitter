...
\label{eq:infprecadd}
\end{equation}
precisions add.  (An electrical picture, exact in form: read each
variance as a resistance and each precision as a conductance;
independent estimates combine like resistors wired in parallel, and
conductances in parallel add.)  A worked number to keep: yesterday says $20.0\%$
...
$\lambda>0$ carries the share $\lambda/(\lambda+\mathcal{I})$ of
...
densely-quoted at-the-money level toward yesterday exactly as
...
move the market has plainly made.  A fitter must report what the
market says; damping a measured move rules the ridge out.
...
requirement line and stay there --- the dead zone.  (b)~The prior's
share of the posterior, \cref{eq:infshare}, with prior
weight $\lambda$ fixed for the ridge (dashed) and gated (solid), both
at unit budget: at twice the required precision the ridge still
carries \MacPriorGateRidgeSharePct\% of the coordinate; the gated
prior carries exactly nothing.}
\label{fig:infgate}
\end{figure}
...
the data pins twice as well as demanded --- the ridge still carries
