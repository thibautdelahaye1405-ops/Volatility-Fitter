...
building the machinery that weighs the two, it pays to ask a static
...
$\lvert k\rvert\le0.10$ and set the rest aside.  This staged morning
is not artificial meanness: on an illiquid name, or in the first
minutes of a session, a band like this is all the market offers,
and the desk still needs a whole smile --- the wings included ---
to price, hedge, and feed every integral of Chapter~5.
...
\Cref{fig:infflat} shows the result, and it is worth walking slowly.
In panel~(a), the orange dots are the morning's quotes and the five
...
that cost).  Same family, same sweep, same machinery --- the only
...
returns to exactly how far it can be trusted --- but it prices the
geometry of panel~(b) without a single refit.
...
says the choice should be disclosed and defensible, and the most
defensible candidate is obvious: yesterday's surface, which was
