\section{Counting once, and testing the stated variances}
\label{sec:infaudit}

Two matters remain: how to let the filter steer a real calibration
without counting any quote twice, and how to test --- against data
--- that the stated variances match the realized errors.
...
implementation is wrong: build the posterior from today's quotes,
then feed that posterior back into the same fit as one more prior
row \emph{alongside those same quotes}.  The quotes then enter twice
--- once through the likelihood, once again inside the
``prior'' --- and the published variance is too small by exactly
...
two-step update is a one-line calculation:
...
in other coordinates, are dropped; what persistence keeps is only what
...
\subsection{The standardized-error test}

A filter always makes charts smoother.  Whether its stated
uncertainty is correct is a separate question, and it has a precise
form: if the posterior variance is correct, the error divided by
its stated scale has standard deviation one.  Define the
\emph{standardized error}
\begin{equation}
\mathcal{Z}
=\frac{\text{truth}-\hat O^{+}}{\sqrt{\mathcal{V}^{+}}},
\label{eq:infz}
\end{equation}
and take its standard deviation over a run of days.  A stated
variance with std$(\mathcal{Z})=1$ is called \emph{consistent}:
it matches the realized errors.  A value well above one
means the filter is overconfident (bars too narrow); well below
one, conservative (bars too wide).  The test needs a truth to
compare against, so it runs wherever one exists --- held-out quotes
on real history, or, as here, a synthetic world where the truth is
known exactly.

\begin{figure}[htbp]
\centering
\paperfig{\textwidth}{fig_flt_audit.pdf}
\caption{The standardized-error test on a seeded synthetic history
(\MacFiltAuditDays\ days; the true level diffuses at
\MacFiltAuditQTrueBp\ bp/$\sqrt{\text{day}}$ and jumps
$+\MacFiltAuditShockPts$ points twice; the filter is told the true
observation noise, wide on every seventh day).  (a)~A window around
a jump: the filter run at the true walk scale (blue, with its
$\pm1$-sd band) follows in two days; the starved budget (violet)
crawls.  (b)~The test curve: std of $\mathcal{Z}$ against the
assumed walk scale --- starved budgets sit far above the dotted
target of $1$ (\MacFiltAuditZstdStarved\ at $10$~bp), the true
scale reads \MacFiltAuditZstdHonest.  (c)~Mean error against days
since the jump: starved \MacFiltAuditDayZeroStarved\ points on the
day and still \MacFiltAuditDayThreeStarved\ three days later; the
true scale \MacFiltAuditDayZeroHonest\ then pennies; the surprise
widening cuts the jump-day error to \MacFiltAuditDayZeroGate.}
\label{fig:infaudit}
\end{figure}

\Cref{fig:infaudit} runs the test end to end on a seeded synthetic
...
observed daily through noise whose true scale the filter is told
...
scales $q_{\mathrm{walk}}$, and every statement of the last two
sections becomes measurable.

Panel~(a) shows the first jump up close.  The filter run at the
true walk scale (blue, its one-standard-deviation band shaded)
...
narrow band, weights its prediction too heavily, and crawls toward
the new level for a week.

Panel~(b) is the test itself: the standard deviation of
$\mathcal{Z}$, \cref{eq:infz}, against the assumed walk scale.  At
the true scale the curve sits near the dotted target ---
std$(\mathcal{Z})=\MacFiltAuditZstdHonest$, the small excess being
the two jumps, which the plain clock term does not model; with the
surprise widening on it drops to \MacFiltAuditZstdGated.  To the
left the curve climbs steeply: at a third of the true scale the
standardized errors have standard deviation
\MacFiltAuditZstdStarved\ --- the stated bars are three times too
narrow.  And panel~(c) shows the same starved filter's
\emph{speed}: \MacFiltAuditDayZeroStarved\ points of error on the
jump day, still \MacFiltAuditDayThreeStarved\ points three days
later, against \MacFiltAuditDayZeroHonest-then-pennies at the true
scale, and \MacFiltAuditDayZeroGate\ on the day with the surprise
widening.  The general lesson runs against intuition: shrinking the
prediction budget does not make a filter cautious.  It makes the
filter \emph{overconfident and slow at once} --- sure of a surface
it updates too slowly --- and std$(\mathcal{Z})$ registers both
symptoms in one number.
...
budget: set by judgment, stated, not derived.\\
\addlinespace
...
The standardized-error test: std$(\mathcal{Z})$
\MacFiltAuditZstdStarved\ starved vs \MacFiltAuditZstdHonest\
at the true scale (\MacFiltAuditZstdGated\ widened); jump-day errors
...
method that publishes uncertainty.\\
\bottomrule
\end{tabular}
\end{table}
...
constraints of Part~I, and the filtered state is read off
arbitrage-checked smiles.  \emph{Information} is the chapter's
subject made explicit: the chapter measured not just what the data
says but where it says \emph{nothing} --- the flat valley --- and
where it speaks with a noisy voice --- the inflated observation
variance.  \emph{Convention} fills the remainder, and every
convention here enters through a stated, testable quantity: a gate
with a dead zone, a variance in stated-noise units, a walk scale
the standardized-error test can reject.  Posterior equals data
where the data speaks, and yesterday --- transported, priced, and
counted once --- where it does not: the thesis of this book, made
dynamic.
