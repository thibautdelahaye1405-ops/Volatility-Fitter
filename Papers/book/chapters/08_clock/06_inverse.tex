...
measures what such an estimator can and cannot promise.
...
What can the size estimator promise?  \Cref{fig:clkident} is the
audit, run on boards where the truth is planted: a flat clock
...
and it is worth deriving because it is the honest price tag of the
sparsity charge.

Work on the audit's configuration: one live candidate $N$ (the
others parked at zero, where the audit's solve indeed leaves them)
...
proportionality (the audit's measured pair,
...
One more audit line completes the promise: handed a perfectly flat
ladder, the solver installs exactly \MacClkIdentFlatDays\ days.  A
sparsity prior's virtue is precisely that silence is its default.

\begin{figure}[htbp]
\centering
\paperfig{\textwidth}{fig_clk_ident.pdf}
\caption{The planted-event audit.  (a)~Recovered vs planted size:
