...
The estimator is audited; point it at the real data.  This section
runs \eqref{eq:clkobj} on the two frozen ladders --- NVDA, where the
chapter began, and SPY --- and reads the results with the discipline
\cref{sec:clkinverse} paid for.

\Cref{fig:clkread}(a) is the disciplined run.  Candidates are allowed
at the first five expiries --- through December 2026, a
\emph{horizon} choice disclosed in the contract --- while the later
...
Now read panel~(a) honestly, in two directions.  What the solver has
...
telling the clock story about \emph{everything}.  A perfectly flat
...
SPY is the audit's flat-ladder promise kept on real data.  The
...
below the day boundary without new machinery: a day becomes a
...
$\mathcal{F}_i$. &
The audit (\cref{fig:clkident}): shrinkage
...
(Chapter~7), and now readings on an honest clock --- the observation
