...
amplitudes it still has to cross.  Nothing shrinks the mean:
...
node's baseline uncertainty enters the machinery exactly once.  For a
...
is \cref{ch:infer}'s update, grown up.
...
The marginal variance $1/p+\beta^{2}v$ is larger, and it is the honest
one: $A$ is not fixed --- it is itself uncertain, and $B$ inherits
that.  The gap between the two is exactly $\beta^{2}v$, the informer's
own doubt.  Reported confidence in this book is \emph{always the
marginal}; quoting the conditional overstates certainty most at
exactly the dark nodes whose informers are themselves uncertain ---
the nodes the machinery exists for.

The objective is written and its output understood.  What remains is
to check that it \emph{accounts} honestly --- that no configuration
