...
with little confidence, or a small amplitude with great confidence.
Any encoding that forces the two into one number loses one of
them.
...
Chapter~8's event logic applied to a scenario).  Differentiate
...
=(\tau_j/\tau_i)(\tau_i/\tau_j)=1$, so a ladder cannot assert
...
is a change of units, not a second belief.  The convention that keeps it
...
largest and desk intuition lives.  One consequence, which
\cref{sec:grbelieve} will lean on: a contract is
...
chooses units; it does not create one-way inference.
Making influence genuinely one-way is a different, stronger assertion,
and it takes a mechanism this section does not have.

\subsection{The contract, tested}

\Cref{fig:grcontract} runs the two-dial contract through the chapter's
