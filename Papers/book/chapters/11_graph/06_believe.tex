...
instant, and the sharpest test is a node that was
...
Now hand the same tape to the solve of
\cref{sec:grmeet} run afresh at each instant, with whoever is printing
right at that moment as the lit set.  (The example is walked in raw
...
Before adding state, close one tempting escape.  After the print,
...
So give each node the state the example demands.  Write $\mathcal{U}_i$
...
systematic prediction ($10-13=-3$ in the example).  Between
...
half-life quoted on one clock --- the universe uses
days, the example above ran in hours.  (And the \emph{variance} of the
...
the decay for three settings of the dial on the example's hourly clock:
...
standardized-error test of \cref{sec:grcomplete} is what checks it.
...
Direction is the remaining assertion, and the algebra cannot supply
it.  When $B$'s surprising print
arrived, the desk revised \emph{B's} state only --- it did not mark
the index down.  That is a structural assertion about the world
...
Concretely: a satellite's print enters the solve as a measurement
...
consequence of it.  The policy is one sentence: a
...
attributable --- which is what lets \cref{sec:grcomplete}
print, for any published mark, exactly where every point of it came
from.  The solve is fully specified; it remains to run it.
