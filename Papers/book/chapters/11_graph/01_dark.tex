...
fails on three counts, and naming them tells us what the machinery
...
Chapter~8's clock.  This is the division of labour promised at the end
...
node $i$, the honest answer is $\Theta_i=0$: publish the baseline,
...
\cref{sec:grcomplete} will hold the machinery to Chapter~10's audit:
the standardized errors $\mathcal{Z}$ of \cref{eq:infz} must scatter
...
into an arbitrage-free smile through the machinery of Part~I, so the
...
zero innovation everywhere and are not solved), so that one field is
the whole story; where a handle-specific point matters, it is
flagged.  We now know
