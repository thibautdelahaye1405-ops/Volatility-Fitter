...
\label{eq:varswap}
\end{equation}
This is the fair variance-swap strike under the usual continuous-path,
continuous-monitoring identity \citep{Neuberger1994,DemeterfiDermanKamalZou1999}.
For a static fitted law it is best read as the log-contract variance; jumps
and discrete sampling require the standard separate corrections.

Equation \eqref{eq:varswap} is the direct-law form of the familiar strip
replication.  The classical construction synthesizes $-\log Y$ from calls and
puts over all strikes; here the law has already been reconstructed, so the
same payoff is integrated against its rank measure.  The two routes agree
when the fitted law and the option strip agree.
...
One build therefore produces prices, digitals, density, the log-contract
variance, and the full parameter Jacobian.  In \cref{sec:calendar} the same
ledger will also compare expiries.

It is useful to count the shared work.  The build stores $(x,x',G,G')$ on one
grid.  A price uses one root and two ledger entries.  A density uses
$\rho/x'$.  The log-contract variance is one weighted sum.  The Jacobian
reads ledger sensitivities at the same roots.  Calendar order compares two
ledger arrays.  These are not five approximations to the law; they are five
queries of the same law.
