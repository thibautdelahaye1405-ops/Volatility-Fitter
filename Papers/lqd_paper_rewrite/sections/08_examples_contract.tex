...
The gallery separates recurring market shape from the extrapolation chosen by
the model.  Through the quoted region the slices follow the term structure of
level and skew.  Outside it the two endpoint speeds complete the curve.  The
median error across these eight slices is \MacSpyGalleryMedianRmsBp\ vol bp;
the worst is \MacSpyGalleryWorstRmsBp\ vol bp.

The short expiries make the informational boundary visible.  Their quoted
spans are narrow in log-moneyness, yet the displayed curves extend far beyond
them.  The sharp local skew is supported by quotes; the remote upward turn is
the exponential-tail completion.  At longer maturities the quote span widens,
and a larger fraction of the displayed smile is directly informed by market
prices.  One family accommodates both cases without changing its validity
mechanism.
...
The model's scope can be stated compactly.

For practical use, a fitted slice should be read in the same order as the
theory.  First inspect quote residuals over the retained span.  Then read the
