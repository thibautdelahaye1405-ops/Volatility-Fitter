...
def binomial_price_batch(
    ...
    def spot_slab(m: int) -> np.ndarray:
        """(n, m+1) spot lattice at step m; same grouping as the scalar:
        s * exp(((2j - m) * sigma) * sqrt(dt))."""
        j = np.arange(m + 1)
        return s * np.exp((2.0 * j - m) * sig_col * sqrt_dt)
...
def deamericanize_batch(
    ...
    def price_at(sig: np.ndarray) -> np.ndarray:
        return binomial_price_batch(ic, s, kk, t, sig, r, q, n_steps)
