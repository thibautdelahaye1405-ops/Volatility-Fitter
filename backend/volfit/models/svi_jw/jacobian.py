...
Two chart entry points share the row assembly logic but not its floating point:
``svi_residual_jacobian`` (the historical raw chart, untouched — the benchmark
harness pins raw and its fits must stay byte-identical) and
``svi_residual_jacobian_structural`` (the R3 structural chart: rows differentiated
in RAW parameter space, then pushed through the chart's closed-form 5x5 chain
matrix ``structural.structural_chain`` — the adoption follow-up, 2026-07-26).
...
# --------------------------------------------- structural chart (R3 follow-up)
def _dw_draw(raw: RawSVI, kk: np.ndarray) -> np.ndarray:
    """``d w(kk) / d (a, b, rho, m, sigma)`` — RAW parameter space, no chart
    chain (the structural path multiplies the 5x5 chain matrix afterwards)."""
    x = kk - raw.m
    s = np.sqrt(x * x + raw.sigma * raw.sigma)
    j = np.empty((kk.size, 5))
    j[:, 0] = 1.0
    j[:, 1] = raw.rho * x + s
    j[:, 2] = raw.b * x
    j[:, 3] = -raw.b * (raw.rho + x / s)
    j[:, 4] = raw.b * raw.sigma / s
    return j


def svi_residual_jacobian_structural(
    theta: np.ndarray,
    k: np.ndarray,
    t: float,
    sqrt_weights: np.ndarray,
    ...
    Same gated row order as the raw version, assembled in raw-parameter space
    and pushed through the chart's chain matrix (``structural_chain``):
    J_theta = J_raw @ d(raw)/d(theta). Under this chart the two penalty rows
    ...
    # branches — with their 1/q — are built only if a hinge somehow fires.
    min_var = raw.a + raw.b * raw.sigma * q
    if min_var < 0.0:
        row_min = -penalty_weight * np.array(
            [1.0, raw.sigma * q, -raw.b * raw.sigma * raw.rho / q, 0.0, raw.b * q]
        )
    else:
        row_min = np.zeros(5)
    wing = raw.b * (1.0 + abs(raw.rho))
    if wing > lee_slope_max:
        row_lee = penalty_weight * np.array(
            [0.0, 1.0 + abs(raw.rho), raw.b * np.sign(raw.rho), 0.0, 0.0]
        )
    else:
        row_lee = np.zeros(5)
    blocks.append(np.vstack([row_min, row_lee]))
    ...
    if cal_k is not None and cal_floor is not None:
        active = (cal_floor - raw.total_variance(cal_k)) > 0.0
        blocks.append((-sqrt_cal * active)[:, None] * _dw_draw(raw, np.asarray(cal_k, float)))

    if ceil_k is not None and ceil_w is not None:
        active = (raw.total_variance(ceil_k) - ceil_w) > 0.0
        blocks.append((sqrt_cal * active)[:, None] * _dw_draw(raw, np.asarray(ceil_k, float)))

    return np.vstack(blocks) @ chain
