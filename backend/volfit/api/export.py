...
# The publish gate lives in export_blockers; re-exported so
# ``export.PublishBlockedError`` / ``export._node_blockers`` keep resolving.
from volfit.api.export_blockers import (  # noqa: F401 — public re-exports
    _PROJECTION_CAL_TOL_BP,
    PublishBlockedError,
    _node_blockers,
    _projection_calendar_audit,
)
...
def _publish(state: AppState, export: SurfaceExport) -> SurfaceExport:
    ...
    try:
        with VolStore(state.store_path) as store:
            snapshot_ids = {
                t.ticker: store.save_snapshot(state.snapshot(t.ticker), source=state.source_of(t.ticker))
