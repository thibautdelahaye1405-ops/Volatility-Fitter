...
def test_debias_holds_discount_debiases_forward_and_smooths():
    """With a reference date the forward is de-biased toward truth and the ATM
    kink vanishes — while the discount is held exactly at the raw parity value
    (so the 1/(D F) IV-level normalization never moves: no level regression)."""
    chain = _american_chain()
    raw = implied_forward(chain, EXPIRY)
    fixed = implied_forward(chain, EXPIRY, REF)
    # Discount is passed through untouched (the deliberate design).
    assert fixed.discount == raw.discount
    # Forward moves toward the truth, not away from it.
