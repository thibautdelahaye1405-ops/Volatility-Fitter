...
- Frontend tests: cd frontend ; npm test   (vitest, 829 tests / 121 files as of 2026-09-24b) ; npm run smoke:ui
            ...
            picker, play / scrub / keys, the Frames stage, the dialog, the
            Surface / Term / Lanes stages, :4197), scripts\series_evidence_check.mjs
            (the Lanes evidence on a PREPARED store — smoke_server.py --db … --tickers
            SPY — screenshots + the evidence table; build the store first, see the
            S5 wrap in ROADMAP, :4198).
- Series (SERIES ARC, 2026-09-10): Docs\series_replay_roadmap.md = the spec (D1–D12
            ...
            2026-09-11: a FRAME BUDGET (`SeriesSpec.frameBudgetSeconds`, 300 s default,
            null = no cap; `volfit.calib.deadline` checked between calibration items and
            before every joint refit of the calendar repair — the desk's Calibrate never
            sets one) bounds a diverging lane (the filter lane had spent 17 min on one
