...
- Tests:    cd backend ; ..\.venv\Scripts\python -m pytest tests -q   (931 passed, 1 skipped as of 2026-07-06, incl. 7 perf rails; +1 live test via $env:VOLFIT_LIVE="1"; perf-only: -m perf -s)
- Parallel calibrate: background Calibrate ships slice fits to a process pool
            ($env:VOLFIT_CALIB_WORKERS, default cpu-1 capped 8; 0/1 = serial,
            byte-identical fits either way; tests/conftest.py pins 1 for the suite).
