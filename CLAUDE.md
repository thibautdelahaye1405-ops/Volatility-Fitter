...
- Tests:    cd backend ; ..\.venv\Scripts\python -m pytest tests -q   (959 tests, 1 skipped as of 2026-07-06, incl. 7 perf rails — NB the graph perf rail needs a quiet box [dense BLAS]; +1 live test via $env:VOLFIT_LIVE="1"; perf-only: -m perf -s)
...
- Frontend tests: cd frontend ; npm test   (vitest, 326 tests) ; npm run smoke:ui
