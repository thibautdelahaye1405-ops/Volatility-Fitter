...
- Tests:    cd backend ; ..\.venv\Scripts\python -m pytest tests -q   (2317 passed / 7 skipped as of 2026-09-09b, ~11 min — split it in two halves [tests/test_[a-k]*.py | test_[l-z]*.py] when a tool caps runs at 10 min, incl. the perf rails — NB the graph perf rail needs a quiet box [dense BLAS]; +1 live test via $env:VOLFIT_LIVE="1"; perf-only: -m perf -s)
...
- Frontend tests: cd frontend ; npm test   (vitest, 661 tests / 93 files) ; npm run smoke:ui
