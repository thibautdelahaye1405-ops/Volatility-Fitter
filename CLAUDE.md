...
- Tests:    cd backend ; ..\.venv\Scripts\python -m pytest tests -q   (1976 passed / 7 skipped as of 2026-08-27, ~12 min, incl. the perf rails — NB the graph perf rail needs a quiet box [dense BLAS]; +1 live test via $env:VOLFIT_LIVE="1"; perf-only: -m perf -s)
...
- Frontend tests: cd frontend ; npm test   (vitest, 346 tests) ; npm run smoke:ui
