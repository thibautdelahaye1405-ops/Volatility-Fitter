...
- Tests:    cd backend ; ..\.venv\Scripts\python -m pytest tests -q   (2454 passed / 7 skipped as of 2026-09-10p, ~14 min — split it in two halves [tests/test_[a-k]*.py | test_[l-z]*.py] when a tool caps runs at 10 min, incl. the perf rails — NB the graph perf rail needs a quiet box [dense BLAS]; +1 live test via $env:VOLFIT_LIVE="1"; perf-only: -m perf -s)
...
- Frontend tests: cd frontend ; npm test   (vitest, 798 tests / 113 files as of 2026-09-10p) ; npm run smoke:ui
